...
| G-17 | timeline conversions | `HardwareSampleTimeline` | **CLOSED (FW-186, T5)**: the timeline accepts every HAL-ladder rate (`HalRateTier`); `NominalBusTicksPerFrame` is 768 at 32 kHz and 0 by design in the 44.1 kHz family (no integral value; conversions are exact rational). Owned by [`HARDWARE_TIMELINE_OWNERSHIP.md`](HARDWARE_TIMELINE_OWNERSHIP.md) |
