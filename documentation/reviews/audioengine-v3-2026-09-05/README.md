...
- [timeline_repro.cpp](/Users/mrmidi/DEV/ASFireWire/documentation/reviews/audioengine-v3-2026-09-05/timeline_repro.cpp) invokes the real timeline, cadence, and geometry policy. It models the production newest-packet selection; it does not compile the ADK observer wrapper. Finding 3 is fixed and its coverage property is now a permanent test; the epoch (finding 5) and safety-plane (finding 2) outputs it prints are still live.
- The DMA reproduction ran the real ring through the host-test fixture and an IIsochDMAMemory decorator that injects deterministic interleavings. Both of its failure signatures (findings 1 and 4) are now fixed, so it has been replaced by `IsochTxPayloadArbitrationTest` in `tests/audio/IsochTxDmaRingTests.cpp`, which uses the same decorator technique and asserts the invariants instead of the bugs. A reproduction for the parts finding 1 does **not** settle — controller prefetch, and controller progress between the fresh position read and the descriptor store — should be written against that fixture.
