...
struct MCPAvcFcpToolsTests {
    private let duetUnitID = UnitInstanceID(
        device: DeviceInstanceID(1),
    ...
    private func fcpAddress() -> ASFWMCPAddress {
        ASFWMCPAddress(deviceInstanceId: DeviceInstanceID(1), nodeId: 0,
                       generation: 17, addressHigh: 0xFFFF, addressLow: 0xF0000B00)
    }
    ...
    @Test func payloadValidationCatchesEmptyAndOversize() {
        #expect(ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .status, payload: [0x01, 0x02]).validationError == nil)
        #expect(ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .status, payload: []).validationError == .malformedRequest)
        let oversize = [UInt8](repeating: 0, count: 513)
        #expect(ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .control, payload: oversize).validationError == .payloadTooLarge)
    }

    @Test func inquiryCommandsAreNotPolicyGated() {
        let request = ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .inquiry, payload: [0x01])
    ...
    @Test func controlCommandsArePolicyGated() throws {
        let request = ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .control, payload: [0x00, 0x11, 0x22, 0x33])
    ...
    @Test func controlCommandRefusedWhenProtocolUnsupported() {
        let request = ASFWMCPFcpCommandRequest(targetUnitID: duetUnitID, address: fcpAddress(), intent: .control, payload: [0x00])
