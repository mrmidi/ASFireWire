...
public:
     ...
     *
     * \param phys Physical address (must be within slab range)
     * \return Virtual address, or nullptr if phys is out-of-bounds
     *
     * \par OHCI Usage
    ...
    /**
     * \brief Publish CPU writes to the DMA mapping before notifying hardware.
     *
     * Copies the requested range into a cache-line-aligned scratch buffer and
     * uses `IODMACommand::PerformOperation(kWrite)` so the device observes the
     * updated data even when the slab is cached.
     */
    void PublishRange(const void* address, size_t length) const noexcept;

    /**
     * \brief Fetch device writes into the CPU cache after DMA completion.
     *
     * Pulls the requested range back through `IODMACommand::PerformOperation(kRead)`
     * to guarantee the CPU sees fresh descriptor or buffer contents.
     */
    ...
    /// Diagnostic: Get cache mode status
    [[nodiscard]] bool IsUncached() const noexcept { return cacheInhibitActive_; }
    ...
    /// True if the current mapping is cache-inhibited
    [[nodiscard]] bool IsCacheInhibitActive() const noexcept { return cacheInhibitActive_; }
...
private:
    ...
    /// Zero the slab respecting cache mode constraints
    void ZeroSlab(size_t length) noexcept;

    /// Ensure coherency scratch buffer is allocated with at least minSize bytes
    [[nodiscard]] bool EnsureScratchBuffer(size_t minSize) const noexcept;

    void TracePublishOrFetch(const char* op,
                             const void* address,
                             size_t requestedLength,
                             size_t alignedOffset,
                             size_t alignedLength,
                             bool performed,
                             kern_return_t kr = kIOReturnSuccess) const noexcept;
    ...
    mutable OSSharedPtr<IOBufferMemoryDescriptor> coherencyScratch_; ///< staging buffer for PerformOperation
    mutable OSSharedPtr<IOMemoryMap> coherencyScratchMap_;           ///< CPU mapping of staging buffer
    mutable size_t coherencyScratchLength_{0};                       ///< size in bytes of staging buffer

    bool cacheInhibitActive_{false}; ///< Mapping is uncached when true
