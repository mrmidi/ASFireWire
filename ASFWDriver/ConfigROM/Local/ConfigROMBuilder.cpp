...
ConfigROMBuilder::ConfigROMBuilder() { Reset(); }

void ConfigROMBuilder::Build(uint32_t busOptions, uint64_t guid, uint32_t nodeCapabilities,
...
bool ConfigROMBuilder::EnsureRootDirectory() {
    ...
    if (!rootDirHeaderIndex_.has_value()) {
...
LeafHandle ConfigROMBuilder::AddTextLeaf(uint8_t key, std::string_view text) {
    ...
    if (!leafHandle.valid()) {
        return invalid; // if failed we leave placeholder (harmless)
    }
    words_[entryIndex] =
        ASFW::FW::MakeDirectoryEntry(key, ASFW::FW::EntryType::kLeaf, leafHandle.offsetQuadlets);
    return leafHandle;
}
...
std::span<const uint32_t> ConfigROMBuilder::ImageBE() const {
    ...
    return {beImage_.data(), quadCount_};
}
...
std::span<const uint32_t> ConfigROMBuilder::ImageNative() const {
    ...
    return {words_.data(), quadCount_};
}

uint32_t ConfigROMBuilder::HeaderQuad() const { return quadCount_ > 0 ? words_[0] : 0; }

uint32_t ConfigROMBuilder::BusInfoQuad() const { return quadCount_ > 2 ? words_[2] : 0; }

uint32_t ConfigROMBuilder::GuidHiQuad() const { return quadCount_ > 3 ? words_[3] : 0; }

uint32_t ConfigROMBuilder::GuidLoQuad() const { return quadCount_ > 4 ? words_[4] : 0; }

void ConfigROMBuilder::Reset() {
    std::ranges::fill(words_, 0U);
    std::ranges::fill(beImage_, 0U);
    quadCount_ = 0;
    rootDirHeaderIndex_.reset();
    lastBusOptions_ = 0U;
    begun_ = false;
    finalized_ = false;
}
...
void ConfigROMBuilder::FinaliseRootDirectory() {
    ...
    const size_t headerIndex = *rootDirHeaderIndex_;
    if (headerIndex >= quadCount_) {
        return;
    }
    const size_t entries = quadCount_ - headerIndex - 1;
    const uint16_t crc = ComputeCRC(headerIndex + 1, entries);
    const uint32_t header = (static_cast<uint32_t>(entries) << 16) | crc;
    words_[headerIndex] = header;
}
