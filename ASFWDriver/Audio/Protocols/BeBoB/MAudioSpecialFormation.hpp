...
[[nodiscard]] constexpr std::optional<MAudioStreamFormation> MAudioFormationFor(
    ...
    const auto captureIndex = static_cast<uint8_t>(captureFormat);
    const auto playbackIndex = static_cast<uint8_t>(playbackFormat);
    if (captureIndex > 1U || playbackIndex > 1U) {
        return std::nullopt;
    }
    ...
    return MAudioStreamFormation{
        .capturePcmChannels = kCapture[captureIndex][*band],
        .playbackPcmChannels = kPlayback[playbackIndex][*band],
