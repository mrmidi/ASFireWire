...
//
// The session store (sessions_ + Load/Store/Get/Clear + epoch/restartId) is a
// separate component (FW-69b). Lifecycle events are protocol-neutral and use
// the Audio log category; the legacy DICE verbosity knob remains the temporary
// shared gate until a dedicated audio setting is added to the user-client ABI.
...
inline void LogFsmEvent(const char* eventName,
    ...
    if (token != 0) {
        ASFW_LOG_V2(Audio,
                    "[FSM] event=%{public}s endpoint=%llu restartId=%llu state=%{public}s phase=%{public}s gen=%u token=%llu reason=%{public}s",
                    eventName,
                    endpointId.value,
                    restartId,
                    ToString(state),
    ...
    ASFW_LOG_V2(Audio,
                "[FSM] event=%{public}s endpoint=%llu restartId=%llu state=%{public}s phase=%{public}s gen=%u reason=%{public}s",
                eventName,
                endpointId.value,
                restartId,
                ToString(state),
...
inline void LogStateTransition(const DuplexRestartSession& session,
    ...
    ASFW_LOG_V2(Audio,
                "[FSM] state %{public}s -> %{public}s endpoint=%llu restartId=%llu phase=%{public}s gen=%u why=%{public}s",
                ToString(oldState),
                ToString(newState),
...
inline void LogPhaseTransition(const DuplexRestartSession& session,
    ...
    ASFW_LOG_V2(Audio,
                "[FSM] phase %{public}s -> %{public}s endpoint=%llu restartId=%llu state=%{public}s gen=%u",
                ToString(oldPhase),
                ToString(newPhase),
                session.endpointId.value,
                session.restartId,
...
inline void LogInvalidation(const DuplexRestartSession& session) noexcept {
    ...
    ASFW_LOG_V3(Audio,
                "[FSM] invalidation class=%{public}s cause=%{public}s retryable=%d status=0x%08x endpoint=%llu restartId=%llu state=%{public}s phase=%{public}s gen=%u",
...
inline void LogRecoveryPolicy(const DuplexRestartSession& session,
                       ...
                       const RecoveryDecision& decision) noexcept {
    ASFW_LOG_V3(Audio,
                "[FSM] policy disposition=%{public}s cause=%{public}s why=%{public}s endpoint=%llu restartId=%llu state=%{public}s phase=%{public}s gen=%u",
...
inline void LogTerminal(const DuplexRestartSession& session) noexcept {
    if (session.state == DuplexRestartState::kFailed && session.lastFailure.has_value()) {
        ...
        ASFW_LOG_V1(Audio,
                    "[FSM] terminal state=%{public}s phase=%{public}s class=%{public}s cause=%{public}s retryable=%d rollback=0x%08x status=0x%08x endpoint=%llu restartId=%llu gen=%u",
                    ToString(session.state),
                    ToString(session.phase),
    ...
    ASFW_LOG_V1(Audio,
                "[FSM] terminal state=%{public}s phase=%{public}s status=0x%08x endpoint=%llu restartId=%llu gen=%u",
                ToString(session.state),
                ToString(session.phase),
