...
//
// Family-neutral duplex lifecycle vocabulary. Device families implement this
// contract; no family owns the session state machine.
...
struct DuplexClockRequestCompletion final {
    uint64_t token{0};
...
struct DuplexRestartIssueInfo final {
...
struct DuplexPrepareResult final {
    FW::Generation generation{0};
...
struct DuplexStageResult final {
    FW::Generation generation{0};
    AudioDuplexChannels channels{};
...
struct DuplexConfirmResult final {
    FW::Generation generation{0};
    AudioDuplexChannels channels{};
    AudioClockConfig appliedClock{};
    AudioStreamRuntimeCaps runtimeCaps{};
...
struct ClockApplyResult final {
    FW::Generation generation{0};
    AudioClockConfig appliedClock{};
    AudioStreamRuntimeCaps runtimeCaps{};
};

struct DuplexHealthResult final {
    FW::Generation generation{0};
    AudioClockConfig appliedClock{};
    AudioStreamRuntimeCaps runtimeCaps{};
...
[[nodiscard]] constexpr bool HasDeviceRestartState(const DuplexRestartSession& session) noexcept {
    return session.ownerClaimed || session.devicePrepared || session.deviceRxProgrammed ||
           session.deviceTxArmed || session.deviceRunning;
}

[[nodiscard]] constexpr bool HasHostRestartState(const DuplexRestartSession& session) noexcept {
    return session.hostDuplexClaimed || session.hostPlaybackReserved ||
           session.hostCaptureReserved || session.hostReceiveStarted ||
...
constexpr void ClearRestartProgress(
