...
void IsochTransmitContext::LogStatistics() const noexcept {
    ...
    // DEBUG-ONLY: periodic IT state logging to help identify patterns in IT failures.
    ASFW_LOG_V3(Isoch, "IT: run=%d active=%d dead=%d evt=0x%02x pkts=%llu IRQ=%llu | CmdPtr=0x%08x Ctrl=0x%08x",
                run,
                active,
                dead,
                eventCode,
