...
void ConfigROMBuilder::Build(uint32_t busOptions,
    ...
    AddImmediateEntry(ROMRootKey::Vendor_ID, vendorId);
    AddImmediateEntry(ROMRootKey::Node_Capabilities, nodeCapabilities);
    if (!vendorName.empty()) {
        AddTextLeaf(ROMRootKey::Vendor_Text, vendorName);
    }
    Finalize();
}
...
bool ConfigROMBuilder::AddImmediateEntry(ROMRootKey key, uint32_t value24) {
    if (!begun_ || finalized_) return false;
    if (!EnsureRootDirectory()) return false;
    if (quadCount_ >= kMaxQuadlets) return false;
    Append(MakeDirectoryEntry(static_cast<uint8_t>(static_cast<uint8_t>(key)), kDirTypeImmediate, value24));
    return true;
}
...
LeafHandle ConfigROMBuilder::AddTextLeaf(ROMRootKey key, std::string_view text) {
    LeafHandle invalid{};
    ...
    words_[entryIndex] = MakeDirectoryEntry(static_cast<uint8_t>(static_cast<uint8_t>(key)), kDirTypeLeaf, leafHandle.offsetQuadlets);
    return leafHandle;
}
