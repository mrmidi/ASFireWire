...
#ifdef ASFW_HOST_TEST
    /// Test hook: override fetch-agent retry budget for subsequently submitted ORBs.
    void SetFetchAgentWriteRetriesForTesting(uint32_t retries) noexcept {
        testFetchAgentWriteRetries_ = retries;
    }

    /// Reset testing override for fetch-agent retries.
    void ResetFetchAgentWriteRetriesForTesting() noexcept {
        testFetchAgentWriteRetries_ = 20;
    }
#endif
...
private:
    ...
    void RefreshCommandBlockAgentAddresses() noexcept;
    ...
    void FailPendingImmediateORBs(int transportStatus, uint8_t sbpStatus) noexcept;
    ...
    /// Write the SBP-2 busy timeout CSR to the target.
    void WriteBusyTimeout() noexcept;

    /// Busy timeout write completion handler.
    void OnBusyTimeoutComplete(uint16_t expectedGeneration,
                               Async::AsyncStatus status,
                               std::span<const uint8_t> response) noexcept;
    ...
    uint32_t busyTimeoutBuffer_{Wire::ToBE32(kBusyTimeoutValue)};
};
