...
bool SBP2LoginSession::Reconnect() noexcept {
    ...
    if (!AllocateResources()) {
        ASFW_LOG(SBP2, "SBP2LoginSession::Reconnect: resource allocation failed");
        SetState(LoginState::Failed);
        return false;
    }
...
bool SBP2LoginSession::AllocateResources() noexcept {
    const bool allResourcesAllocated = loginORBHandle_ != 0 &&
                                      loginResponseHandle_ != 0 &&
                                      statusBlockHandle_ != 0 &&
                                      reconnectORBHandle_ != 0 &&
                                      logoutORBHandle_ != 0;
    const bool anyResourceAllocated = loginORBHandle_ != 0 ||
                                     loginResponseHandle_ != 0 ||
                                     statusBlockHandle_ != 0 ||
                                     reconnectORBHandle_ != 0 ||
                                     logoutORBHandle_ != 0;
    ...
    if (anyResourceAllocated) {
        ASFW_LOG(SBP2,
                 "SBP2LoginSession: inconsistent resource state before allocation "
                 "loginORB=0x%llu loginResp=0x%llu status=0x%llu reconnect=0x%llu logout=0x%llu",
                 static_cast<unsigned long long>(loginORBHandle_),
                 static_cast<unsigned long long>(loginResponseHandle_),
                 static_cast<unsigned long long>(statusBlockHandle_),
                 static_cast<unsigned long long>(reconnectORBHandle_),
                 static_cast<unsigned long long>(logoutORBHandle_));
        DeallocateResources();
    }
    ...
    if (!AllocateLoginResponseAddressSpace()) {
        ASFW_LOG(SBP2, "SBP2LoginSession: rollback login resource allocation at login response stage");
        DeallocateResources();
        return false;
    }
    if (!AllocateStatusBlockAddressSpace()) {
        ASFW_LOG(SBP2, "SBP2LoginSession: rollback login resource allocation at status block stage");
        DeallocateResources();
        return false;
    }
    if (!AllocateReconnectORBAddressSpace()) {
        ASFW_LOG(SBP2, "SBP2LoginSession: rollback login resource allocation at reconnect ORB stage");
        DeallocateResources();
        return false;
    }
    if (!AllocateLogoutORBAddressSpace()) {
        ASFW_LOG(SBP2, "SBP2LoginSession: rollback login resource allocation at logout ORB stage");
        DeallocateResources();
        return false;
    }
...
void SBP2LoginSession::RefreshCommandBlockAgentAddresses() noexcept {
    commandBlockAgent_.nodeID = loginNodeID_;
...
void SBP2LoginSession::OnLogoutWriteComplete(uint16_t expectedGeneration,
    ...
    if (status != Async::AsyncStatus::kSuccess) {
        ...
        ASFW_LOG(SBP2, "SBP2LoginSession: logout transport failed (was loginID=%u)", oldLoginID);
...
void SBP2LoginSession::CompleteLoginFromStatusBlock(const Wire::StatusBlock& block,
    ...
    RefreshCommandBlockAgentAddresses();

    // If unsolicited status was requested while not logged in, enable it after
    // all CBA-derived registers are available.
...
void SBP2LoginSession::ClearORBTracking(bool cancelTimers) noexcept {
    const bool shouldCancelFetchAgentWrite =
        cancelTimers && fetchAgentWriteInUse_ && static_cast<bool>(fetchAgentWriteHandle_);
    const Async::AsyncHandle fetchAgentWriteHandle = fetchAgentWriteHandle_;
    const bool shouldCancelDoorbellWrite =
        cancelTimers && doorbellInProgress_ && static_cast<bool>(doorbellWriteHandle_);
    const Async::AsyncHandle doorbellWriteHandle = doorbellWriteHandle_;
    ...
    if (shouldCancelFetchAgentWrite) {
        (void)bus_.Cancel(fetchAgentWriteHandle);
    }
    if (shouldCancelDoorbellWrite) {
        (void)bus_.Cancel(doorbellWriteHandle);
    }
}
...
void SBP2LoginSession::FailPendingImmediateORBs(int transportStatus,
                                                uint8_t sbpStatus) noexcept {
    auto pending = pendingImmediateORBs_;
    for (auto* orb : pending) {
        FailSubmittedORB(orb, transportStatus, sbpStatus);
    }
}
...
void SBP2LoginSession::WriteBusyTimeout() noexcept {
    ...
    if (!busyTimeoutWriteHandle_) {
        ASFW_LOG(SBP2, "SBP2LoginSession::WriteBusyTimeout: WriteBlock failed");
        busyTimeoutInProgress_ = false;
    }
}

void SBP2LoginSession::OnBusyTimeoutComplete(uint16_t expectedGeneration,
                                             Async::AsyncStatus status,
    ...
    if (status != Async::AsyncStatus::kSuccess && status != Async::AsyncStatus::kAborted) {
        ASFW_LOG(SBP2,
                 "SBP2LoginSession::OnBusyTimeoutComplete: status=%s",
                 Async::ToString(status));
    }
}
