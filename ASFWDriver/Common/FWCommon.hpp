...
inline const char* RespName(Response r) {
    ...
    case Response::Complete:
        return "Complete";
    case Response::ConflictError:
        return "Conflict";
    case Response::DataError:
        return "DataError";
    case Response::TypeError:
        return "TypeError";
    case Response::AddressError:
        return "AddressError";
    case Response::BusReset:
        return "BusReset";
...
[[nodiscard]] inline Ack AckFromByte(uint8_t byte) {
    ...
    case 0x06:
        return Ack::BusyB;
    case 0x0D:
        return Ack::DataError;
    case 0x0E:
        return Ack::TypeError;
    default:
        return Ack::Unknown;
    }
}
...
[[nodiscard]] constexpr uint32_t EncodeBusOptions(const BusOptionsDecoded& in) noexcept {
    uint32_t out = 0;
    if (in.irmc)
        out |= BusOptionsFields::kIRMCMask;
    if (in.cmc)
        out |= BusOptionsFields::kCMCMask;
    if (in.isc)
        out |= BusOptionsFields::kISCMask;
    if (in.bmc)
        out |= BusOptionsFields::kBMCMask;
    if (in.pmc)
        out |= BusOptionsFields::kPMCMask;
...
constexpr inline uint32_t MakeDirectoryEntry(uint8_t key, uint8_t type, uint32_t value24) {
    return (static_cast<uint32_t>(type) & 0x3u) << 30 | (static_cast<uint32_t>(key) & 0x3Fu) << 24 |
           (value24 & 0x00FFFFFFu);
}
} // namespace ASFW::FW
