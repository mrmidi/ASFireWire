...
//
// DeviceIdentityMatch.hpp - Declarative match clauses over Config-ROM identity evidence.
//
// This is the matching primitive the audio device catalog is built from. It reads
// Discovery::DeviceIdentityEvidence directly, which is a deliberate reversal of the
// rule stated in DeviceProfileTypes.hpp ("intentionally free of Discovery types"):
// that rule forced every matcher to work from a flattened scalar query, and the
// flattening is precisely what loses the information the catalog needs — a model_id
// of 0 that was published versus one that was never published, and which of several
// unit directories a specifier/version pair came from.
//
// Matching is AND across the stated fields of one clause; a definition ORs its
// clauses. Missing evidence never satisfies a constrained field, so a device that
// publishes nothing matches nothing — vendor-wide fallback has to be spelled out
// rather than happening by omission.
...
struct IdentityMatchClause final {
    ...
    [[nodiscard]] bool Matches(const Discovery::DeviceIdentityEvidence& device,
        ...
        if (busInfoWord) {
            ...
            // rawBusInfoQuadlets is big-endian wire order (DiscoveryTypes.hpp:141).
            // ASFW::Driver::FromBusOrder is the repo's named helper but lives in
            // Phy/PhyPackets.hpp; DeviceProfiles must not include a PHY header to
            // perform one byte swap. This is the same expansion.
