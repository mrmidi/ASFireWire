...
- DICE: the sample rates offered to macOS now come from the device's own list of supported rates, up to 48 kHz, instead of a fixed 44.1 and 48 kHz. Every device we have recordings of still offers exactly 44.1 and 48 kHz. A device that also supports 32 kHz now offers it, and a rate the device does not list is refused before anything is sent to it.
