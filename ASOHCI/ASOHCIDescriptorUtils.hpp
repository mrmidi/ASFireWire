...
static inline uint32_t DescGetReqCount(uint32_t q0) { return (q0 >> 16) & 0xFFFF; }

// Descriptor quadlet 0 field setting
static inline uint32_t DescSetCmd(uint32_t q0, uint32_t cmd) { return (q0 & ~0xF) | (cmd & 0xF); }
static inline uint32_t DescSetKey(uint32_t q0, uint32_t key) { return (q0 & ~0x70) | ((key & 0x7) << 4); }
static inline uint32_t DescSetInterrupt(uint32_t q0, uint32_t i) { return (q0 & ~0x300) | ((i & 0x3) << 8); }
static inline uint32_t DescSetBranch(uint32_t q0, uint32_t b) { return (q0 & ~0xC00) | ((b & 0x3) << 10); }
static inline uint32_t DescSetReqCount(uint32_t q0, uint32_t reqCount) { return (q0 & ~0xFFFF0000) | ((reqCount & 0xFFFF) << 16); }