...
//
// Spec refs (OHCI 1.1): Chapter 6 (IsoRxIntEvent demux), §10.3 (context discovery),
...
public:
    ...
    virtual kern_return_t StartReception(uint32_t ctxId,
    ...
    virtual kern_return_t EnqueueDualBufferReceive(uint32_t ctxId,
                                                  const IRDualBufferInfo& dualBufferInfo,
                                                  const IRQueueOptions& opts);

    // Top-half: called from the device's main ISR after reading host IntEvent (§6.4)
...
protected:
    ...
    virtual kern_return_t BuildStandardProgram(IRMode mode,
                                             const void* bufferVAs[],
                                             const uint32_t bufferPAs[],
                                             const uint32_t bufferSizes[],
                                             uint32_t bufferCount,
    ...
    virtual kern_return_t BuildDualBufferProgram(const IRDualBufferInfo& info,
                                                const IRQueueOptions& opts,
