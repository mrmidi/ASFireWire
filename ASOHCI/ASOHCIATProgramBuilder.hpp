...
//
// Spec refs: OHCI 1.1 §7.7 (Descriptor fields), §7.5 (interrupt policy ‘i’ bits)
...
public:
    ...
    static inline uint32_t EncodeInterrupt(uint32_t interrupt) {
        return (interrupt << ATDescField::kInterruptShift) & ATDescField::kInterruptMask;
    }
    
    static inline uint32_t EncodeBranchAddr(uint32_t addr, uint32_t z) {
        return ((addr << ATDescField::kBranchAddrShift) & ATDescField::kBranchAddrMask) |
               ((z << ATDescField::kZNibbleShift) & ATDescField::kZNibbleMask);
    }
    
public:
public:
    // Resets internal state and reserves space for the worst-case descriptor count
    // (header immediate + N payload fragments + last).
    // Pool provides aligned memory; we return an ATDesc::Program on Finalize().
    ...
    virtual void AddHeaderImmediate(const uint32_t* header, uint32_t headerBytes, ATIntPolicy ip);

    // Append a payload fragment. The builder decides OUTPUT_MORE vs LAST placement.
