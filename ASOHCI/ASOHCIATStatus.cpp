...
static inline ATEvent _EventFromXfer(uint16_t xferStatus) {
  ...
  switch (evt) {
  case 0x0:
    return ATEvent::kUnknown; // no explicit event encoded here
  case 0x2:
    return ATEvent::kMissingAck;
  case 0x3:
    return ATEvent::kUnderrun;
  case 0x4:
    return ATEvent::kUnknown; // overrun not typical on AT
  case 0x6:
    return ATEvent::kDataRead;
  case 0x7:
    return ATEvent::kUnknown; // data write (builder specific)
  case 0x8:
    return ATEvent::kUnknown; // bus reset handled elsewhere
  case 0x9:
    return ATEvent::kTimeout;
  case 0xE:
    return ATEvent::kFlushed;
  default:
    return ATEvent::kUnknown;
  }
}
...
Completion ATStatus::Decode(const HWStatusWord &s) {
  ...
  case 0x0:
    c.ack = ATAck::kComplete;
    break;
  case 0x1:
    c.ack = ATAck::kPending;
    break;
  case 0x2:
  case 0x3:
  case 0x4:
    ...
    break;
  case 0x6:
    c.ack = ATAck::kDataError;
    break;
  default:
    c.ack = ATAck::kUnknown;
    break;
  }
...
ATAck ATStatus::ToAck(const Completion &c) { return c.ack; }

bool ATStatus::IsSuccess(const Completion &c) {
  // Success if ACK is complete or pending; ignore event unless it indicates
  // timeout/flush
  bool ackOK = (c.ack == ATAck::kComplete || c.ack == ATAck::kPending);
  if (!ackOK)
    return false;
  if (c.event == ATEvent::kTimeout || c.event == ATEvent::kFlushed)
    return false;
  return true;
}
