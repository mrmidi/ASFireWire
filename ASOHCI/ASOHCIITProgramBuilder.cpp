                                                ...
                                                bool isLast)
{
    ...
    if (_descUsed + 2 > _blk.descriptorCount) return;

    _ip = ip;

    ATDesc::Descriptor* im0 = static_cast<ATDesc::Descriptor*>(_blk.virtualAddress) + _descUsed;
    ATDesc::Descriptor* im1 = im0 + 1;
    memset(im0, 0, sizeof(*im0));
    memset(im1, 0, sizeof(*im1));

    uint32_t cmd = isLast ? ITDescOps::kCmd_OUTPUT_LAST : ITDescOps::kCmd_OUTPUT_MORE;
    uint32_t b = isLast ? 0x3 : 0x0;
    uint32_t i = mapInterruptPolicy(ip);
    uint32_t reqCount = 8; // 2 quadlets header

    uint32_t ctrl0 = (cmd & 0xF) | ((ITDescOps::kKey_IMMEDIATE & 0x7) << 4) | ((i & 0x3) << 8) | ((b & 0x3) << 10) | ((reqCount & 0xFFFF) << 16);
    im0->quad[0] = ctrl0;
    im0->quad[1] = 0; // Reserved for IT

    uint32_t hdr0 = ((uint32_t)sy & 0xF) << 12 | (0xA << 8) | ((uint32_t)channel & 0x3F) << 2 | ((uint32_t)tag & 0x3);
...
void ASOHCIITProgramBuilder::AddPayloadFragment(uint32_t payloadPA, uint32_t payloadBytes, bool isLast)
{
    if (!_pool || !_blk.valid) return;
    ...
    uint32_t ctrl0 = (cmd & 0xF) | ((b & 0x3) << 10) | ((i & 0x3) << 8) | ((payloadBytes & 0xFFFF) << 16);
    d->quad[0] = ctrl0;
    d->quad[1] = payloadPA;
    d->quad[2] = 0;
    d->quad[3] = 0;
...
ITDesc::Program ASOHCIITProgramBuilder::Finalize()
{
    ...
    if (DescGetKey(lastDesc->quad[0]) == ITDescOps::kKey_IMMEDIATE) {
        p.tailPA = _blk.physicalAddress + ((_descUsed - 2) * sizeof(ATDesc::Descriptor));
        p.tailVA = static_cast<uint8_t*>(_blk.virtualAddress) + ((_descUsed - 2) * sizeof(ATDesc::Descriptor));
        
        ATDesc::Descriptor* im1 = static_cast<ATDesc::Descriptor*>(_blk.virtualAddress) + (_descUsed - 1);
