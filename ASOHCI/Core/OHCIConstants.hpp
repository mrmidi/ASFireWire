...
static constexpr uint32_t kOHCI_ConfigROMmap =
    0x034; // Config ROM base address (device view)

// Host Control
static constexpr uint32_t kOHCI_HCControl = 0x048; // Base
...
// Some code refers to kOHCI_IntEvent for a read of the raw event register at
// 0x080. OHCI defines separate write-1-to-set / write-1-to-clear views; reading
// either address (0x080) yields current event bits. Provide both names for
// clarity.
...
static constexpr uint32_t kOHCI_Int_ReqTxComplete =
    0x00000001; // Bit 0: AT request DMA complete (§7.6)
static constexpr uint32_t kOHCI_Int_RespTxComplete =
    0x00000002; // Bit 1: AT response DMA complete (§7.6)
static constexpr uint32_t kOHCI_Int_ARRQ =
    0x00000004; // Bit 2: AR request DMA complete (§8.6)
static constexpr uint32_t kOHCI_Int_ARRS =
    0x00000008; // Bit 3: AR response DMA complete (§8.6)
static constexpr uint32_t kOHCI_Int_RqPkt =
    0x00000010; // Bit 4: AR request packet received (§8.6)
static constexpr uint32_t kOHCI_Int_RsPkt =
    0x00000020; // Bit 5: AR response packet received (§8.6)
static constexpr uint32_t kOHCI_Int_IsochTx =
    0x00000040; // Bit 6: Isochronous transmit (§6.3)
static constexpr uint32_t kOHCI_Int_IsochRx =
    0x00000080; // Bit 7: Isochronous receive (§6.4)

// Error Condition Interrupts (OHCI 1.1 §6.1 Table 6-1 bits 8-9)
static constexpr uint32_t kOHCI_Int_PostedWriteErr =
    0x00000100; // Bit 8: Posted write error (§13.2.8.1)
static constexpr uint32_t kOHCI_Int_LockRespErr =
    0x00000200; // Bit 9: Lock response error (§5.5.1)

// Bits 10-14: Reserved per OHCI 1.1 specification

// Self-ID and Bus Management Interrupts (OHCI 1.1 §6.1 Table 6-1 bits 15-17)
static constexpr uint32_t kOHCI_Int_SelfIDComplete2 =
    0x00008000; // Bit 15: Secondary Self-ID complete (§11.5)
static constexpr uint32_t kOHCI_Int_SelfIDComplete =
    0x00010000; // Bit 16: Self-ID complete (§11.5)
static constexpr uint32_t kOHCI_Int_BusReset =
    0x00020000; // Bit 17: PHY bus reset mode (§6.1.1)

// System and PHY Interrupts (OHCI 1.1 §6.1 Table 6-1 bits 18-19)
static constexpr uint32_t kOHCI_Int_RegAccessFail =
    0x00040000; // Bit 18: Register access failed (missing SCLK)
static constexpr uint32_t kOHCI_Int_Phy =
    0x00080000; // Bit 19: PHY status transfer request

// Cycle Timer and Management Interrupts (OHCI 1.1 §6.1 Table 6-1 bits 20-25)
static constexpr uint32_t kOHCI_Int_CycleSynch =
    0x00100000; // Bit 20: New isochronous cycle started
static constexpr uint32_t kOHCI_Int_Cycle64Seconds =
    0x00200000; // Bit 21: 64-second counter bit 7 changed
static constexpr uint32_t kOHCI_Int_CycleLost =
    0x00400000; // Bit 22: No cycle_start between cycleSynch events
static constexpr uint32_t kOHCI_Int_CycleInconsistent =
    0x00800000; // Bit 23: Cycle timer mismatch (§5.13)
static constexpr uint32_t kOHCI_Int_UnrecoverableError =
    0x01000000; // Bit 24: Context dead or fatal error
static constexpr uint32_t kOHCI_Int_CycleTooLong =
    0x02000000; // Bit 25: Cycle >120μsec (root node only)

// High-Order Interrupts (OHCI 1.1 §6.1 Table 6-1 bits 26-31)
static constexpr uint32_t kOHCI_Int_PhyRegRcvd =
    0x04000000; // Bit 26: PHY register packet received
static constexpr uint32_t kOHCI_Int_AckTardy =
    0x08000000; // Bit 27: Late acknowledgment received
static constexpr uint32_t kOHCI_Int_SoftInterrupt =
    0x10000000; // Bit 28: Software-generated interrupt
static constexpr uint32_t kOHCI_Int_VendorSpecific =
    0x20000000; // Bit 29: Implementation-specific interrupt
// Bit 30: Reserved per OHCI 1.1 specification
static constexpr uint32_t kOHCI_Int_MasterEnable =
    0x80000000; // Bit 31: Master interrupt enable
...
// NOTE: The OHCI spec does NOT define separate "Asynchronous Response" filter
// registers; earlier aliases named AsRspFilter* were incorrect and have been
// removed.
static constexpr uint32_t kOHCI_AsReqFilterHiSet = 0x0100;
static constexpr uint32_t kOHCI_AsReqFilterHiClear =
    0x0104; // read returns HiClear, write clears bits
...
static constexpr uint32_t kOHCI_IsoXmitContextBase(uint32_t n) {
  return 0x0200 + 0x10 * n;
}
static constexpr uint32_t kOHCI_IsoXmitContextControlSet(uint32_t n) {
  return kOHCI_IsoXmitContextBase(n) + 0x00;
} // Set @ base
static constexpr uint32_t kOHCI_IsoXmitContextControlClear(uint32_t n) {
  return kOHCI_IsoXmitContextBase(n) + 0x04;
} // Clear @ +0x04
static constexpr uint32_t kOHCI_IsoXmitCommandPtr(uint32_t n) {
  return kOHCI_IsoXmitContextBase(n) + 0x0C;
}

// Isochronous Transmit ContextControl fields (subset) per §9.2:
// CycleMatch value in [28:16], enable bit nearby (commonly bit 29 on
// implementations). Keep these symbolic; controller-specific variants may
// exist.
static constexpr uint32_t kOHCI_IT_CycleMatchMask =
    (0x1FFFu << 16); // 13 bits 0..7999 (§9.2)
static constexpr uint32_t kOHCI_IT_CycleMatchEnable =
    (1u << 29); // enable (symbolic, §9.2)

// ------------------------ Isochronous Receive Contexts (OHCI 1.1 §10.3)
// --------- IR context n register block: base 0x0400 + 0x20*n
static constexpr uint32_t kOHCI_IsoRcvContextBase(uint32_t n) {
  return 0x0400 + 0x20 * n;
}
static constexpr uint32_t kOHCI_IsoRcvContextControlSet(uint32_t n) {
  return kOHCI_IsoRcvContextBase(n) + 0x00;
} // Set @ base
static constexpr uint32_t kOHCI_IsoRcvContextControlClear(uint32_t n) {
  return kOHCI_IsoRcvContextBase(n) + 0x04;
} // Clear @ +0x04
static constexpr uint32_t kOHCI_IsoRcvContextMatch(uint32_t n) {
  return kOHCI_IsoRcvContextBase(n) + 0x10;
} // Match @ +0x10
static constexpr uint32_t kOHCI_IsoRcvCommandPtr(uint32_t n) {
  return kOHCI_IsoRcvContextBase(n) + 0x0C;
} // CommandPtr @ +0x0C

// IR ContextControl fields per §10.3:
// Multi-channel mode enable and bufferFill mode (implementation-specific bit
// positions)
static constexpr uint32_t kOHCI_IR_MultiChannelMode =
    (1u << 28); // Enable multi-channel reception (symbolic, §10.3)
static constexpr uint32_t kOHCI_IR_BufferFill =
    (1u << 31); // Enable buffer-fill mode (symbolic, §10.3)

// IR ContextMatch register fields per §10.3:
static constexpr uint32_t kOHCI_IR_ChannelMask =
    0x0000003F; // Bits 5-0: Channel number (0-63)
static constexpr uint32_t kOHCI_IR_TagMask =
    0x000000C0; // Bits 7-6: Tag field match
static constexpr uint32_t kOHCI_IR_SyncMask =
    0x00000F00; // Bits 11-8: Sync field match

// Legacy context control bit (deprecated - use kOHCI_ContextControl_*
// definitions above)
static constexpr uint32_t kOHCI_Context_Run = (1u << 15);

// ------------------------ Self‑ID parse (IEEE 1394-2008 Alpha §16.3.2.1)
// --------------------------- Top two bits identify self-ID quadlets: b31..b30
// == 10b
...
static constexpr uint32_t kSelfID_IsExtended_Mask =
    0x00800000; // b23: 0=#0, 1=#1/#2

// Packet #0 fields (Table 16-4)
static constexpr uint32_t kSelfID_LinkActive_Mask = 0x00400000; // L, b22
static constexpr uint32_t kSelfID_GapCount_Mask =
    0x003F8000; // gap_cnt, b21..b16
static constexpr uint32_t kSelfID_GapCount_Shift = 16;
static constexpr uint32_t kSelfID_Speed_Mask =
    0x0000C000; // sp, b15..b14 (00=S100,01=S200,10=S400,11=reserved)
...
static constexpr uint32_t kSelfID_PowerClass_Mask =
    0x00000700; // pwr, b10..b8 (Alpha 3/7/15W)
...
static constexpr uint32_t kOHCI_NodeID_idValid =
    0x80000000; // alias to Int_MasterEnable
...
static constexpr uint32_t kOHCI_PhyControl_rdDone =
    0x80000000; // Bit 31: read completion status
static constexpr uint32_t kOHCI_PhyControl_rdReg =
    0x00008000; // Bit 15: initiate read request
static constexpr uint32_t kOHCI_PhyControl_wrReg =
    0x00004000; // Bit 14: initiate write request
// Field masks
static constexpr uint32_t kOHCI_PhyControl_rdAddr_Mask =
    0x0F000000; // Bits 27-24: read address
static constexpr uint32_t kOHCI_PhyControl_rdData_Mask =
    0x00FF0000; // Bits 23-16: read data
static constexpr uint32_t kOHCI_PhyControl_regAddr_Mask =
    0x000007C0; // Bits 10-6: register address
static constexpr uint32_t kOHCI_PhyControl_wrData_Mask =
    0x000000FF; // Bits 7-0: write data
...
static constexpr uint32_t kOHCI_PhyControl_WritePending =
    kOHCI_PhyControl_wrReg;
...
static constexpr uint8_t kPHY_REG_1 =
    1; // PHY register 1 address (vendor/ID/reset bits)
static constexpr uint8_t kPHY_INITIATE_BUS_RESET =
    0x40; // IBR bit: Initiate Bus Reset (IEEE 1394a)

// ------------------------ OHCI DMA Context Control Bits (OHCI 1.1 §3.1.1)
// ------------------------ ContextControl register bit definitions - used by
// all DMA contexts
static constexpr uint32_t kOHCI_ContextControl_run =
    0x00008000; // Bit 15: Enable descriptor processing
static constexpr uint32_t kOHCI_ContextControl_wake =
    0x00000400; // Bit 10: Resume/continue processing
static constexpr uint32_t kOHCI_ContextControl_dead =
    0x00000800; // Bit 11: Fatal error occurred
static constexpr uint32_t kOHCI_ContextControl_active =
    0x00000200; // Bit 9: Currently processing descriptors

// Event code field (bits 4-0) per OHCI 1.1 Table 3-2
static constexpr uint32_t kOHCI_ContextControl_evtCode_Mask =
    0x0000001F; // Bits 4-0: Event code
static constexpr uint32_t kOHCI_ContextControl_spd_Mask =
    0x000000E0; // Bits 7-5: Speed indication
...
static constexpr uint32_t kOHCI_EvtCode_DescriptorRead =
...
// ------------------------ All descriptors are 16-byte aligned and use quadlet
// (32-bit) fields
...
struct OHCI_ARInputMoreDescriptor {
  ...
  uint32_t i : 2; // Interrupt control (0x3=interrupt on completion, 0x0=no
                  // interrupt)
  ...
  uint32_t
      branchAddress : 28; // Next descriptor block address (16-byte aligned)
...
static constexpr uint32_t kOHCI_Z_EndOfProgram =
    0; // Last descriptor in program
static constexpr uint32_t kOHCI_Z_MinPacketSize =
    2; // Minimum packet size (2 blocks)
static constexpr uint32_t kOHCI_Z_MaxPacketSize =
    8; // Maximum packet size (8 blocks)

// Descriptor alignment requirements
static constexpr size_t kOHCI_DescriptorAlign =
    16; // All descriptors 16-byte aligned
static constexpr size_t kOHCI_DescriptorSize = 16; // Standard descriptor size
static constexpr size_t kOHCI_ImmediateDescSize =
    32; // *-Immediate descriptor size
