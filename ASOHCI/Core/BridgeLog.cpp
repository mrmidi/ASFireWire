...
kern_return_t bridge_log_copy(OSData **outData) {
  ...
  *outData = nullptr;
  ...
  const size_t maxLines = (seqNow < BRIDGE_LOG_CAPACITY)
                              ? (size_t)seqNow
                              : (size_t)BRIDGE_LOG_CAPACITY;
  if (maxLines == 0) {
  ...
  uint64_t minSeq =
      (seqNow > BRIDGE_LOG_CAPACITY) ? (seqNow - BRIDGE_LOG_CAPACITY) : 0;
