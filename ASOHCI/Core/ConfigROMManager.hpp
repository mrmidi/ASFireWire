...
public:
  ...
  // Allocate/map ROM buffer, build image, DMA-map, and program ROM map
  // register. romBytes usually 1024. Returns kIOReturnSuccess / error
  // (implementation).
  ...
  // Call from BusReset handling to atomically commit staged BusOptions and
  // Header.
...
private:
  void buildAndStage(uint32_t busOptions, uint32_t guidHi, uint32_t guidLo,
                     uint32_t romBytes);
  void programROMMap();
