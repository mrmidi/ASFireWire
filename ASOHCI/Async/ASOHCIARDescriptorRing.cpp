...
//  - §8.2  AR context registers (As Req/Rsp Rcv Context
//  Base/Control/CommandPtr)
...
kern_return_t ASOHCIARDescriptorRing::Initialize(IOPCIDevice *pci,
  ...
  if (!_impl->bufMD || !_impl->bufMap || !_impl->bufDMA || !_impl->bufSeg)
    return kIOReturnNoMemory;

  kern_return_t kr = kIOReturnSuccess;
  for (uint32_t i = 0; i < _impl->buffers; ++i) {
    kr = IOBufferMemoryDescriptor::Create(
        kIOMemoryDirectionInOut, _impl->bufSize, 4, /* quadlet alignment */
        &_impl->bufMD[i]);
    if (kr != kIOReturnSuccess || !_impl->bufMD[i])
      return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;

    kr = _impl->bufMD[i]->CreateMapping(0, 0, 0, 0, 0, &_impl->bufMap[i]);
    if (kr != kIOReturnSuccess || !_impl->bufMap[i])
      return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;

    IODMACommandSpecification spec{};
    spec.options = kIODMACommandSpecificationNoOptions;
    spec.maxAddressBits = 32;
    kr = IODMACommand::Create(_impl->pci, kIODMACommandCreateNoOptions, &spec,
                              &_impl->bufDMA[i]);
    if (kr != kIOReturnSuccess || !_impl->bufDMA[i])
      return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;
  ...
  const size_t chainBytes =
      (size_t)_impl->buffers * sizeof(OHCI_ARInputMoreDescriptor);
  kr = IOBufferMemoryDescriptor::Create(kIOMemoryDirectionInOut, chainBytes,
                                        kOHCI_DescriptorAlign, &_impl->descsMD);
  if (kr != kIOReturnSuccess || !_impl->descsMD)
    return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;

  kr = _impl->descsMD->CreateMapping(0, 0, 0, 0, 0, &_impl->descsMap);
  if (kr != kIOReturnSuccess || !_impl->descsMap)
    return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;
  ...
  kr = IODMACommand::Create(_impl->pci, kIODMACommandCreateNoOptions, &dspec,
                            &_impl->descsDMA);
  if (kr != kIOReturnSuccess || !_impl->descsDMA)
    return (kr != kIOReturnSuccess) ? kr : kIOReturnNoMemory;
  ...
  kr = _impl->descsDMA->PrepareForDMA(kIODMACommandPrepareForDMANoOptions,
                                      _impl->descsMD, 0, chainBytes, &dflags,
                                      &dsegCount, dsegs);
  if (kr != kIOReturnSuccess || dsegCount < 1 || dsegs[0].address == 0)
    return (kr != kIOReturnSuccess) ? kr : kIOReturnNoResources;
  ...
  os_log(ASLog(), "ARring: init ok (%u buffers x %u bytes) DMA=0x%llx",
         _impl->buffers, _impl->bufSize,
         (unsigned long long)_impl->descsDMABase);
...
kern_return_t ASOHCIARDescriptorRing::Deallocate() {
  ...
  if (_impl->descsMap) {
    _impl->descsMap->release();
    _impl->descsMap = nullptr;
  }
  if (_impl->descsMD) {
    _impl->descsMD->release();
    _impl->descsMD = nullptr;
  }
  ...
  if (_impl->bufDMA) {
    ...
    delete[] _impl->bufDMA;
    _impl->bufDMA = nullptr;
  }
  if (_impl->bufMap) {
    for (uint32_t i = 0; i < _impl->buffers; ++i) {
      if (_impl->bufMap[i])
        _impl->bufMap[i]->release();
    }
    delete[] _impl->bufMap;
    _impl->bufMap = nullptr;
  }
  if (_impl->bufMD) {
    for (uint32_t i = 0; i < _impl->buffers; ++i) {
      if (_impl->bufMD[i])
        _impl->bufMD[i]->release();
    }
    delete[] _impl->bufMD;
    _impl->bufMD = nullptr;
  }
...
kern_return_t
ASOHCIARDescriptorRing::GetCommandPtrSeed(uint32_t *outDescriptorAddress,
                                          uint8_t *outZ) const {
  if (!_impl || !_impl->armed || _impl->descsDMABase == 0)
    return kIOReturnNotReady;
  if (outDescriptorAddress)
    *outDescriptorAddress = (uint32_t)_impl->descsDMABase;
  if (outZ)
    *outZ = 1; // one descriptor in each block
  return kIOReturnSuccess;
}

bool ASOHCIARDescriptorRing::TryPopCompleted(ARPacketView *outView,
                                             uint32_t *outRingIndex) {
  if (!_impl || !_impl->armed)
    return false;
  ...
  if (!_impl->haveReady) {
    ...
    for (uint32_t n = 0; n < _impl->buffers; ++n) {
      ...
      uint32_t req = d->reqCount;
      if (req == 0 || req > _impl->bufSize)
        req = _impl->bufSize;
      uint32_t res = d->resCount;
      if (res < req) {
        _impl->haveReady = true;
        _impl->readyIndex = idx;
        break;
      }
  ...
  if (outView) {
    ...
    uint32_t res = d->resCount;
    outView->data =
        (_impl->bufMap && _impl->bufMap[idx])
            ? (const uint8_t *)(uintptr_t)_impl->bufMap[idx]->GetAddress()
            : nullptr;
...
kern_return_t ASOHCIARDescriptorRing::Recycle(uint32_t ringIndex) {
  if (!_impl || !_impl->armed || ringIndex >= _impl->buffers)
    return kIOReturnBadArgument;
  ...
  d->resCount = req;
  d->xferStatus = 0;
  if (_impl->consumeIdx == ringIndex)
    _impl->consumeIdx = (_impl->consumeIdx + 1U) % _impl->buffers;
  return kIOReturnSuccess;
}
...
uint32_t ASOHCIARDescriptorRing::BufferCount() const {
  return _impl ? _impl->buffers : 0;
}
uint32_t ASOHCIARDescriptorRing::BufferBytes() const {
  return _impl ? _impl->bufSize : 0;
}
ARBufferFillMode ASOHCIARDescriptorRing::FillMode() const {
  return _impl ? _impl->fill : ARBufferFillMode::kImmediate;
}
