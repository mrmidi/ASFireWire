...
//  Manages a pool of OUTPUT_* descriptors (DMA-coherent), hands out 16B-aligned
//  blocks.
//
//  Spec refs: OHCI 1.1 §7.1 (CommandPtr & Z nibble), §7.6 (prefetch/pipeline
//  fetch boundaries)
...
public:
  ...
  // Allocate a descriptor block (N descriptors * 16B). Returns contiguous
  // chunk.
...
private:
  ...
  static constexpr size_t kMaxAllocation =
      16 * 1024 * 1024; // 16MB limit like Linux
