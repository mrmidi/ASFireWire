...
ASOHCIATContext::ASOHCIATContext()
    : fPCIDevice(nullptr), fContextType(AT_REQUEST_CONTEXT), fBARIndex(0),
      fContextBaseOffset(0), fContextControlSetOffset(0),
      fContextControlClearOffset(0), fCommandPtrOffset(0),
      fDescriptorPool(nullptr), fDescriptorPoolMap(nullptr),
      fDescriptorPoolAddress(nullptr), fDescriptorPoolSize(0),
      fInitialized(false), fRunning(false) {}
...
kern_return_t ASOHCIATContext::Initialize(IOPCIDevice *pciDevice,
...
kern_return_t ASOHCIATContext::Start() {
  ...
  // Set run bit to enable context (but it will be inactive until packets are
  // queued)
...
kern_return_t ASOHCIATContext::QueuePacket(PacketType packetType,
  ...
  os_log(ASLog(),
         "ASOHCIATContext: QueuePacket placeholder - type=%u, headerSize=%u, "
         "payloadSize=%u",
...
kern_return_t ASOHCIATContext::WriteContextControl(uint32_t value,
                                                   bool setRegister) {
  uint32_t offset =
      setRegister ? fContextControlSetOffset : fContextControlClearOffset;
...
kern_return_t ASOHCIATContext::WriteCommandPtr(uint32_t descriptorAddress,
                                               uint32_t zValue) {
