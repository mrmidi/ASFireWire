...
//
//  Spec refs: OHCI 1.1 §7.3 (ATRetries), §7.4 (FairnessControl), §7.6 (in-order
//  vs pipelined)
...
struct ATRetryPolicy {
  ...
  // Time-limit fields (dual-phase retry window) - optional, may be read-only 0
  // if not implemented
  uint32_t secondLimit = 0; // 3 bits: seconds counter (0-7s)
  uint32_t cycleLimit =
      0; // 13 bits: 1394 cycle counter (0-7999, 8000 cycles = 1s)
  ...
  uint32_t maxPhyResp =
      0; // maxPhysRespRetries: Physical Response Unit retry cap
...
struct ATPipelinePolicy {
  // If true, we allow multiple outstanding and accept out-of-order completions
  // (§7.6)
