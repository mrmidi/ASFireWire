...
// Based on OHCI 1.1 Specification §7.1 (AT DMA Context Programs), §7.7 (Data
// formats)
//
...
void ASOHCIATProgramBuilder::Begin(ASOHCIATDescriptorPool &pool,
  ...
  // Per OHCI §7.1: Maximum 7 fragments per packet (1 header + 5 payload + 1
  // last)
  uint32_t reserveCount = (maxDescriptors == 0) ? 7 : maxDescriptors;
  if (reserveCount > 7) {
    os_log(ASLog(), "ASOHCIATProgramBuilder: Invalid maxDescriptors %u (max 7)",
           maxDescriptors);
    reserveCount = 7;
  }
  ...
  if (!_blk.valid) {
    os_log(
        ASLog(),
        "ASOHCIATProgramBuilder: Failed to allocate %u descriptors from pool",
        reserveCount);
    return;
  }

  os_log(ASLog(),
         "ASOHCIATProgramBuilder: Reserved %u descriptors (PA=0x%x, Z=%u)",
         reserveCount, _blk.physicalAddress, _blk.zValue);
}

void ASOHCIATProgramBuilder::AddHeaderImmediate(const uint32_t *header,
                                                uint32_t headerBytes,
                                                ATIntPolicy ip) {
  if (!_pool || !_blk.valid || !header) {
    os_log(ASLog(),
           "ASOHCIATProgramBuilder: Invalid state for header immediate");
    return;
  }

  // Per OHCI §7.1: First descriptor must be *-Immediate with full 1394 packet
  // header Header size must be 8, 12, or 16 bytes (2, 3, or 4 quadlets)
  if (headerBytes != 8 && headerBytes != 12 && headerBytes != 16) {
    os_log(
        ASLog(),
        "ASOHCIATProgramBuilder: Invalid header size %u (must be 8, 12, or 16)",
        headerBytes);
    return;
  }
  ...
  os_log(
      ASLog(),
      "ASOHCIATProgramBuilder: Added header immediate (%u bytes, %u quadlets)",
      headerBytes, quadlets);
}

void ASOHCIATProgramBuilder::AddPayloadFragment(uint32_t payloadPA,
                                                uint32_t payloadBytes) {
  if (!_pool || !_blk.valid) {
    os_log(ASLog(),
           "ASOHCIATProgramBuilder: Invalid state for payload fragment");
    return;
  }

  if (payloadBytes == 0) {
    os_log(ASLog(),
           "ASOHCIATProgramBuilder: Skipping zero-length payload fragment");
    return;
  }
  ...
  os_log(ASLog(),
         "ASOHCIATProgramBuilder: Added payload fragment (PA=0x%x, %u bytes)",
...
ATDesc::Program ASOHCIATProgramBuilder::Finalize() {
  ...
  if (!_pool || !_blk.valid || _descUsed == 0) {
    os_log(ASLog(), "ASOHCIATProgramBuilder: Cannot finalize - invalid state "
                    "or no descriptors");
    return program;
  }
  ...
  // Check if last descriptor was OUTPUT_MORE-Immediate (key=2) or OUTPUT_MORE
  // (key=0)
  ...
  lastDesc->quad[0] = (lastDesc->quad[0] & ~ATDescField::kCmdMask) |
                      EncodeCmd(ATDescField::kCmdOutputLast);
  ...
  if (isImmediate) {
    ...
    switch (_ip) {
    case ATIntPolicy::kAlways:
      interruptBits = 0x3;
      break; // 2'b11
    case ATIntPolicy::kInterestingOnly:
      interruptBits = 0x1;
      break; // 2'b01
    }

    lastDesc->quad[0] |=
        EncodeInterrupt(interruptBits) |            // i field [22:21]
    ...
    lastDesc->quad[2] =
        (lastDesc->quad[2] &
         0xFFFF0000) |          // Preserve upper 16 bits (xferStatus)
    ...
    // Fourth quadlet: xferStatus (upper 16) + timeStamp (lower 16) - will be
    // filled by hardware
    lastDesc->quad[3] = 0;
  }
  ...
  os_log(ASLog(),
         "ASOHCIATProgramBuilder: Finalized program with %u descriptors "
         "(head=0x%x, tail=0x%x, Z=%u)",
...
void ASOHCIATProgramBuilder::Cancel() {
  if (_pool && _blk.valid) {
    os_log(ASLog(),
           "ASOHCIATProgramBuilder: Canceling program build, returning %u "
           "descriptors to pool",
