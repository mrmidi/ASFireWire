...
kern_return_t ASOHCIARContext::Initialize(IOPCIDevice *pci, uint8_t barIndex,
                                          ARContextRole role,
...
kern_return_t ASOHCIARContext::Initialize(IOPCIDevice *pci,
  ...
  ARContextRole role = (contextType == AR_REQUEST_CONTEXT)
                           ? ARContextRole::kRequest
                           : ARContextRole::kResponse;
  ...
  kern_return_t kr =
      Initialize(pci, barIndex, role, offs, ARBufferFillMode::kImmediate);
  ...
  if (!_ring) {
    ...
    kr = r->Initialize(pci, bufferCount, bufferBytes,
                       ARBufferFillMode::kImmediate);
    if (kr != kIOReturnSuccess) {
      delete r;
      return kr;
    }
...
kern_return_t ASOHCIARContext::Initialize(IOPCIDevice *pci,
  ...
  return Initialize(pci, contextType, barIndex, kDefaultBufCount,
                    kDefaultBufBytes);
}
...
kern_return_t ASOHCIARContext::Stop() { return ASOHCIContextBase::Stop(); }
...
bool ASOHCIARContext::TryDequeue(ARPacketView *outView,
