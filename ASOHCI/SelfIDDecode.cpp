...
static inline PortCode toPortCode(uint32_t twoBits) {
  switch (twoBits & 0x3u) {
    case 0: return PortCode::NotPresent;
    case 1: return PortCode::NotActive;
    case 2: return PortCode::Parent;
    case 3: return PortCode::Child;
  }
...
Result Decode(const uint32_t* buffer, uint32_t quadletCount)
{
  ...
  // OHCI 1.1 §11.3: buffer[0] is header (generation|timestamp). Generation often matches SelfIDCount bits [23:16].
  ...
  // Build a simple map phyId -> partial AlphaRecord (first occurrence wins for base packet)
  struct Partial { AlphaRecord rec; bool haveBase=false; uint8_t nextPort=3; };
  ...
  for (size_t n = 0; n < sidIdx.size(); ++n) {
    ...
    if (!isExt) {
      ...
      p.rec.gapCount   = static_cast<uint8_t>((q & kSelfID_GapCount_Mask) >> kSelfID_GapCount_Shift);
      p.rec.speed      = static_cast<LinkSpeed>((q & kSelfID_Speed_Mask) >> kSelfID_Speed_Shift);
      p.rec.delay      = (q & kSelfID_Delay_Mask) != 0;
      p.rec.contender  = (q & kSelfID_Contender_Mask) != 0;
      p.rec.powerClass = static_cast<uint8_t>((q & kSelfID_PowerClass_Mask) >> 8);
      ...
      if (!p.haveBase) {
        ...
        out.warnings.push_back({"Orphan extended Self-ID for PHY " + std::to_string(phy)});
      }
  ...
  for (unsigned phy = 0; phy < 64; ++phy) {
    if (tmp[phy].haveBase) out.nodes.push_back(tmp[phy].rec);
  }
