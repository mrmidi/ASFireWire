...
// IR uses same 16B INPUT_MORE/INPUT_LAST descriptor format as AT for standard descriptors.
// DUALBUFFER descriptors are 32B and specific to IR dual-buffer mode.
//
// Spec refs (OHCI 1.1): §10.1 (IR DMA Context Programs), Table 10-1 (INPUT_MORE/INPUT_LAST),
...
namespace IRDesc {
    // INPUT_MORE and INPUT_LAST use identical layout to AT descriptors (OHCI Table 10-1)
    ...
    struct DualBufferDescriptor {
        ...
        uint32_t firstReqCount_secondReqCount;   // bits 31:16 = firstReqCount, bits 15:0 = secondReqCount
        uint32_t branchAddress_Z;                // bits 31:4 = branchAddress, bits 3:0 = Z
        uint32_t firstResCount_secondResCount;   // bits 31:16 = firstResCount, bits 15:0 = secondResCount
        ...
        void SetControl(bool statusEnable, uint8_t interruptPolicy, uint8_t branchControl, 
                       uint8_t waitControl, uint16_t firstSize) {
            quad0 = ((statusEnable ? 1 : 0) << 31) |
                   (0x0 << 28) |                    // key = 0 (Table 10-2)
                   ((interruptPolicy & 0x3) << 26) |
                   ((branchControl & 0x3) << 24) |
                   ((waitControl & 0x3) << 22) |
                   (firstSize & 0xFFFF);
        }
        
        void SetCounts(uint16_t firstReq, uint16_t secondReq) {
            firstReqCount_secondReqCount = ((uint32_t)firstReq << 16) | (secondReq & 0xFFFF);
        }
        ...
        void InitializeResCounts(uint16_t firstReq, uint16_t secondReq) {
            firstResCount_secondResCount = ((uint32_t)firstReq << 16) | (secondReq & 0xFFFF);
        }
    };
}
...
namespace IRDescOps {
    ...
    static constexpr uint32_t kKey_STANDARD       = 0x0;  // Standard key for INPUT_MORE/INPUT_LAST
    
    // Branch control values (OHCI Table 10-1)
    static constexpr uint32_t kBranch_Never       = 0x0;  // INPUT_MORE in packet-per-buffer
    static constexpr uint32_t kBranch_Always      = 0x3;  // INPUT_LAST and buffer-fill mode
