...
// Optional: initiate a PHY bus reset to kick off Self-ID on otherwise idle buses
if (ivars->phyAccess) {
    kern_return_t rbr = ivars->phyAccess->updatePhyRegisterWithMask(kPHY_REG_1, kPHY_INITIATE_BUS_RESET, kPHY_INITIATE_BUS_RESET);
    if (rbr == kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCI: Initiated PHY bus reset (IBR set) to start Self-ID");
    } else {
        os_log(ASLog(), "ASOHCI: WARN: Failed to initiate PHY bus reset: 0x%08x", rbr);
    }
}
