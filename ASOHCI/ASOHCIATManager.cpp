...
// Based on OHCI 1.1 Specification §7.4 (AT Retries), §7.5 (Fairness), §7.7 (AT Pipelining)
//
                                         ...
                                         const ATPipelinePolicy& pipe)
{
    ...
    if (result != kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCIATManager: Failed to initialize Request context: 0x%x", result);
        return result;
    }
    ...
    if (result != kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCIATManager: Failed to initialize Response context: 0x%x", result);
        return result;
    }
...
kern_return_t ASOHCIATManager::Start()
{
    ...
    result = _req.Start();
    if (result != kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCIATManager: Failed to start Request context: 0x%x", result);
        return result;
    }
    ...
    result = _rsp.Start();
    if (result != kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCIATManager: Failed to start Response context: 0x%x", result);
...
kern_return_t ASOHCIATManager::Stop()
{
    ...
    if (_pci) {
        ...
        if (reqResult != kIOReturnSuccess) {
            os_log(ASLog(), "ASOHCIATManager: Failed to stop Request context: 0x%x", reqResult);
        }
        
        rspResult = _rsp.Stop();
        if (rspResult != kIOReturnSuccess) {
            os_log(ASLog(), "ASOHCIATManager: Failed to stop Response context: 0x%x", rspResult);
        }
...
kern_return_t ASOHCIATManager::QueueRequest(const uint32_t* header, uint32_t headerBytes,
                                           ...
                                           const ATQueueOptions& opts)
{
    ...
    if (headerBytes != 8 && headerBytes != 12 && headerBytes != 16) {
        os_log(ASLog(), "ASOHCIATManager: Invalid header size %u for request", headerBytes);
        return kIOReturnBadArgument;
    }
    ...
    if (maxDescriptors > 7) {
        os_log(ASLog(), "ASOHCIATManager: Too many fragments %u (max 5)", fragments);
        return kIOReturnBadArgument;
    }
    ...
    os_log(ASLog(), "ASOHCIATManager: Queued request with %u fragments", fragments);
    return kIOReturnSuccess;
}

kern_return_t ASOHCIATManager::QueueResponse(const uint32_t* header, uint32_t headerBytes,
                                            const uint32_t* payloadPAs, const uint32_t* payloadSizes, uint32_t fragments,
                                            const ATQueueOptions& opts)
{
    if (!_pci || !header) {
    ...
    // Validate header size per OHCI §7.1
    if (headerBytes != 8 && headerBytes != 12 && headerBytes != 16) {
        os_log(ASLog(), "ASOHCIATManager: Invalid header size %u for response", headerBytes);
        return kIOReturnBadArgument;
    }
    ...
    if (result != kIOReturnSuccess) {
        os_log(ASLog(), "ASOHCIATManager: Failed to enqueue response: 0x%x", result);
        return result;
    }
    
    os_log(ASLog(), "ASOHCIATManager: Queued response with %u fragments", fragments);
    return kIOReturnSuccess;
}
...
void ASOHCIATManager::OnBusResetBegin()
{
    ...
    os_log(ASLog(), "ASOHCIATManager: Bus reset begin - stopping AT transmission");
}
...
void ASOHCIATManager::OnBusResetEnd()
{
    ...
    os_log(ASLog(), "ASOHCIATManager: Bus reset end - AT contexts ready for restart");
