                                          ...
                                          const ARFilterOptions& /*filterOpts*/)
{
    ...
    if (bufferCount < 2U || bufferCount > 64U) return kIOReturnBadArgument;
    ...
    // Create contexts
    if (!_arReq) _arReq = new ASOHCIARContext();
    if (!_arRsp) _arRsp = new ASOHCIARContext();
    if (!_arReq || !_arRsp) return kIOReturnNoMemory;

    // Create rings
    if (!_ringReq) _ringReq = new ASOHCIARDescriptorRing();
    if (!_ringRsp) _ringRsp = new ASOHCIARDescriptorRing();
    if (!_ringReq || !_ringRsp) return kIOReturnNoMemory;

    // Initialize rings (shared policy for now)
    kern_return_t kr = _ringReq->Initialize(pci, bufferCount, bufferBytes, fillMode);
    if (kr != kIOReturnSuccess) return kr;
    kr = _ringRsp->Initialize(pci, bufferCount, bufferBytes, fillMode);
    if (kr != kIOReturnSuccess) return kr;

    // Initialize contexts with role-specific offsets
    kr = _arReq->Initialize(pci, barIndex, ARContextRole::kRequest, OffsetsFor(ARContextRole::kRequest), fillMode);
    if (kr != kIOReturnSuccess) return kr;
    kr = _arRsp->Initialize(pci, barIndex, ARContextRole::kResponse, OffsetsFor(ARContextRole::kResponse), fillMode);
    if (kr != kIOReturnSuccess) return kr;
    ...
    os_log(ASLog(), "ARManager: initialized (%u buffers × %u bytes, BAR=%u)", bufferCount, bufferBytes, _bar);
    return kIOReturnSuccess;
}

kern_return_t ASOHCIARManager::Start()
{
    if (!_arReq || !_arRsp) return kIOReturnNotReady;
    kern_return_t kr = _arReq->Start();
    if (kr != kIOReturnSuccess) return kr;
    kr = _arRsp->Start();
    if (kr != kIOReturnSuccess) { (void)_arReq->Stop(); return kr; }
...
kern_return_t ASOHCIARManager::Stop()
{
    ...
    if (_arReq) r1 = _arReq->Stop();
    if (_arRsp) r2 = _arRsp->Stop();
    return (r1 != kIOReturnSuccess) ? r1 : r2;
}

void ASOHCIARManager::SetPacketCallback(PacketCallback cb, void* refcon)
{
    _cb = cb; _cbRefcon = refcon;
}

// IRQ fan-in (router would call these)
void ASOHCIARManager::OnRequestPacketIRQ()  { if (_arReq) _arReq->OnPacketArrived(); }
void ASOHCIARManager::OnResponsePacketIRQ() { if (_arRsp) _arRsp->OnPacketArrived(); }
void ASOHCIARManager::OnRequestBufferIRQ()  { if (_arReq) _arReq->OnBufferComplete(); }
void ASOHCIARManager::OnResponseBufferIRQ() { if (_arRsp) _arRsp->OnBufferComplete(); }

// Optional pull model
bool ASOHCIARManager::DequeueRequest(ARPacketView* outView, uint32_t* outIndex)
{ return _arReq ? _arReq->TryDequeue(outView, outIndex) : false; }

bool ASOHCIARManager::DequeueResponse(ARPacketView* outView, uint32_t* outIndex)
