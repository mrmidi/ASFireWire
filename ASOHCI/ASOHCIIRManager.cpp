...
//
// Spec refs (OHCI 1.1): Chapter 6 (IsoRxIntEvent/Mask demux), §10.2-10.6 (IR contexts),
//   §10.3 (context discovery), §10.5 (interrupt semantics), §10.6 (data formats)
//
...
kern_return_t ASOHCIIRManager::Initialize(IOPCIDevice* pci,
                                          uint8_t barIndex,
                                          const IRPolicy& defaultPolicy)
{
    ...
    os_log(ASLog(), "IRManager: Initialize (bar=%u, dynamic allocation) contexts=%u", _bar, _numCtx);
...
kern_return_t ASOHCIIRManager::StartReception(uint32_t ctxId,
                                             const IRChannelFilter& channelFilter,
                                             ...
                                             void* callbackContext)
{
    ...
    if (!completionCallback) return kIOReturnBadArgument;
    ...
    state.active = true;
    os_log(ASLog(), "IRManager: ctx%u reception started mode=%u", ctxId, (uint32_t)queueOpts.receiveMode);
    return kIOReturnSuccess;
}
...
kern_return_t ASOHCIIRManager::EnqueueReceiveBuffers(uint32_t ctxId,
...
{
    if (ctxId >= _numCtx) return kIOReturnBadArgument;
    if (!bufferVAs || !bufferPAs || !bufferSizes || bufferCount == 0) return kIOReturnBadArgument;
    if (!_pool.IsInitialized()) return kIOReturnNotReady;
    
    auto& state = _contextStates[ctxId];
    if (!state.active) return kIOReturnNotReady;
    
    // Build receive program for the specified mode
    IRDesc::Program program;
    kern_return_t r = BuildStandardProgram(opts.receiveMode, bufferVAs, bufferPAs, bufferSizes, bufferCount, opts, &program);
    if (r != kIOReturnSuccess) {
        return r;
    }
    ...
    os_log(ASLog(), "IRManager: ctx%u enqueued %u buffers mode=%u", ctxId, bufferCount, (uint32_t)opts.receiveMode);
    return kIOReturnSuccess;
}

kern_return_t ASOHCIIRManager::EnqueueDualBufferReceive(uint32_t ctxId,
...
kern_return_t ASOHCIIRManager::RefillContext(uint32_t ctxId)
{
    ...
    auto& pool = _bufferPools[ctxId];
    if (pool.bufferCount == 0) return kIOReturnNoResources;
    ...
    if (availableCount == 0) return kIOReturnNoResources;
    
    // Re-enqueue available buffers (simplified)
    os_log(ASLog(), "IRManager: ctx%u refill with %u buffers", ctxId, availableCount);
    return kIOReturnSuccess;
}

bool ASOHCIIRManager::ContextNeedsRefill(uint32_t ctxId) const
{
    if (ctxId >= _numCtx) return false;
...
uint32_t ASOHCIIRManager::ProbeContextCount()
{
    ...
    // Strategy: write all-ones to mask set, read back to see which bits hardware implements
...
kern_return_t ASOHCIIRManager::BuildStandardProgram(IRMode mode,
                                                   const void* bufferVAs[],
                                                   const uint32_t bufferPAs[],
                                                   const uint32_t bufferSizes[],
                                                   uint32_t bufferCount,
...
{
    if (!outProgram) return kIOReturnBadArgument;
    ...
    switch (mode) {
        case IRMode::kBufferFill:
            ...
            r = builder.BuildBufferFillProgram(bufferPAs[0], bufferSizes[0], opts, outProgram);
            break;
            
        case IRMode::kPacketPerBuffer:
            r = builder.BuildPacketPerBufferProgram(bufferPAs, bufferSizes, bufferCount, opts, outProgram);
            break;
...
kern_return_t ASOHCIIRManager::BuildDualBufferProgram(const IRDualBufferInfo& info,
                                                     const IRQueueOptions& opts,
