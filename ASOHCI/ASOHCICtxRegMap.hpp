...
// Sources: OHCI 1.1 §4.2 Table 4-3 (addresses for Async/IT/IR contexts).
// [oai_citation:1‡42-register-map.pdf](file-service://file-P79PQ5V6pmT1XpGoSNzZeP)
...
public:
  ...
  static bool Compute(ASContextKind kind, uint32_t index,
                      ASContextOffsets *out) {
    if (!out)
      return false;
    uint32_t base = 0;

    switch (kind) {
    case ASContextKind::kAT_Request: // §4.2: 0x0180 block; Set=+0x08,
                                     // Clear=+0x04, CmdPtr=+0x0C.
      base = kATReqTxBase;
      break;
    case ASContextKind::kAT_Response: // §4.2: 0x01A0 block; Set=+0x08,
                                      // Clear=+0x04, CmdPtr=+0x0C.
      base = kATRspTxBase;
      break;
    case ASContextKind::kAR_Request: // §4.2: 0x01C0 block; Set=+0x08,
                                     // Clear=+0x04, CmdPtr=+0x0C.
      base = kATReqRxBase;
      break;
    case ASContextKind::kAR_Response: // §4.2: 0x01E0 block; Set=+0x08,
                                      // Clear=+0x04, CmdPtr=+0x0C.
      base = kATRspRxBase;
      break;
    case ASContextKind::kIT_Transmit: // §4.2: 0x0200 + 16*n; Set=base,
                                      // Clear=base+0x04, CmdPtr=base+0x0C.
      if (index >= 32)
        return false;
      base = kITBase0 + kITStride * index;
      break;
    case ASContextKind::kIR_Receive: // §4.2: 0x0400 + 32*n; Set=base,
                                     // Clear=base+0x04, CmdPtr=base+0x0C.
      if (index >= 32)
        return false;
    ...
    if (kind == ASContextKind::kIT_Transmit ||
        kind == ASContextKind::kIR_Receive) {
