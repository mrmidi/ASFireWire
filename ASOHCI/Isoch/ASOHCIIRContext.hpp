...
public:
  // ctxIndex: hardware IR context number (0..N-1). Offsets computed in .cpp
  // (see §10.3)
  virtual kern_return_t Initialize(IOPCIDevice *pci, uint8_t barIndex,
                                   uint32_t ctxIndex);

  // Override Start: configure channel filtering and sync matching before
  // starting
  ...
  EnqueueDualBuffer(const IRProgram::DualBufferProgram &program,
                    const IRQueueOptions &opts);

  // Called by manager when isoRxIntEvent indicates this context fired (§10.5;
  // demux via Chapter 6)
  virtual void OnInterruptRx();

  // Expose completion callback and statistics
  virtual void SetCompletionCallback(void (*callback)(const IRCompletion &,
                                                      void *),
                                     void *context);
...
protected:
  virtual void
  RecoverDeadContext() override; // handle buffer overrun recovery (§10.5)
...
private:
  ...
  kern_return_t ConfigureContextMatch(uint8_t channel, uint8_t tag,
                                      uint8_t sync);
