...
//   §10.6 (data formats), Table 10-1 (INPUT descriptors), Table 10-2
//   (DUALBUFFER)
...
void ASOHCIIRProgramBuilder::Begin(ASOHCIATDescriptorPool &pool,
...
kern_return_t ASOHCIIRProgramBuilder::BuildBufferFillProgram(
    uint32_t bufferPA, uint32_t bufferSize, const IRQueueOptions &opts,
    IRDesc::Program *outProgram) {
  if (!_pool || !_blk.virtualAddress || !outProgram)
    return kIOReturnBadArgument;
  if (_descUsed >= _blk.descriptorCount)
    return kIOReturnNoSpace;

  // Buffer-Fill mode uses single INPUT_LAST descriptor (§10.2.1)
  IRDesc::Descriptor *desc =
      &static_cast<IRDesc::Descriptor *>(_blk.virtualAddress)[_descUsed];

  BuildInputLast(bufferPA, bufferSize, opts.interruptPolicy, opts.syncMatch,
                 opts.includeHeader, opts.includeTimestamp,
  ...
  os_log(ASLog(),
         "IRProgramBuilder: Built buffer-fill program - bufferPA=0x%x, size=%u",
         bufferPA, bufferSize);

  return kIOReturnSuccess;
}

kern_return_t ASOHCIIRProgramBuilder::BuildPacketPerBufferProgram(
    const uint32_t bufferPAs[], const uint32_t bufferSizes[],
    uint32_t bufferCount, const IRQueueOptions &opts,
    IRDesc::Program *outProgram) {
  if (!_pool || !_blk.virtualAddress || !outProgram)
    return kIOReturnBadArgument;
  if (bufferCount == 0 || bufferCount > _blk.descriptorCount)
    return kIOReturnBadArgument;
  if (_descUsed + bufferCount > _blk.descriptorCount)
    return kIOReturnNoSpace;

  IRDesc::Descriptor *descriptors =
      &static_cast<IRDesc::Descriptor *>(_blk.virtualAddress)[_descUsed];

  // Build INPUT_MORE descriptors for all but last buffer
  for (uint32_t i = 0; i < bufferCount - 1; i++) {
    BuildInputMore(bufferPAs[i], bufferSizes[i],
                   IRIntPolicy::kNever, // Only interrupt on last
                   opts.syncMatch, opts.includeHeader, &descriptors[i]);
  }

  // Build INPUT_LAST descriptor for final buffer
  uint32_t lastIdx = bufferCount - 1;
  BuildInputLast(bufferPAs[lastIdx], bufferSizes[lastIdx], opts.interruptPolicy,
                 opts.syncMatch, opts.includeHeader, opts.includeTimestamp,
                 0, // No branch for final descriptor
                 &descriptors[lastIdx]);

  // Link descriptors into chain
  LinkDescriptors(descriptors, bufferCount,
                  _blk.physicalAddress +
                      (_descUsed * sizeof(IRDesc::Descriptor)));
  ...
  // Build program structure
  outProgram->headPA = _blk.physicalAddress +
                       (_descUsed - bufferCount) * sizeof(IRDesc::Descriptor);
  outProgram->tailPA =
      _blk.physicalAddress + (_descUsed - 1) * sizeof(IRDesc::Descriptor);
  ...
  os_log(ASLog(),
         "IRProgramBuilder: Built packet-per-buffer program - buffers=%u",
         bufferCount);

  return kIOReturnSuccess;
}

kern_return_t ASOHCIIRProgramBuilder::BuildDualBufferProgram(
    const IRDualBufferInfo &info, uint32_t descriptorCount,
    const IRQueueOptions &opts, IRProgram::DualBufferProgram *outProgram) {
  if (!_pool || !_blk.virtualAddress || !outProgram)
    return kIOReturnBadArgument;
  if (descriptorCount == 0 || descriptorCount > (_blk.descriptorCount / 2))
    return kIOReturnBadArgument;

  // DUALBUFFER descriptors are 32 bytes each (double size)
  uint32_t dualBufferSpace =
      descriptorCount * 2; // Each dual-buffer needs 2 regular descriptor slots
  if (_descUsed + dualBufferSpace > _blk.descriptorCount)
    return kIOReturnNoSpace;

  IRDesc::DualBufferDescriptor *dualDescriptors =
      reinterpret_cast<IRDesc::DualBufferDescriptor *>(
          &static_cast<IRDesc::Descriptor *>(_blk.virtualAddress)[_descUsed]);
  ...
  for (uint32_t i = 0; i < descriptorCount; i++) {
    ...
    if (i < descriptorCount - 1) {
      branchAddress = _blk.physicalAddress +
                      (_descUsed + (i + 1) * 2) * sizeof(IRDesc::Descriptor);
      zValue = IRDescOps::kDualBuffer_Continue;
    }

    BuildDualBuffer(info,
                    (i == descriptorCount - 1) ? opts.interruptPolicy
                                               : IRIntPolicy::kNever,
                    opts.syncMatch, branchAddress, zValue, &dualDescriptors[i]);
  }
  ...
  outProgram->headPA = _blk.physicalAddress + (_descUsed - dualBufferSpace) *
                                                  sizeof(IRDesc::Descriptor);
  outProgram->tailPA =
      _blk.physicalAddress + (_descUsed - 2) * sizeof(IRDesc::Descriptor);
  ...
  os_log(ASLog(),
         "IRProgramBuilder: Built dual-buffer program - descriptors=%u, "
         "firstSize=%u",
...
void ASOHCIIRProgramBuilder::BuildInputMore(
    uint32_t bufferPA, uint32_t reqCount, IRIntPolicy intPolicy,
    IRSyncMatch syncMatch, bool includeHeader, IRDesc::Descriptor *outDesc) {
  if (!outDesc)
    return;
  ...
  outDesc->quad[0] =
      (IRDescOps::kCmd_INPUT_MORE << 28) | (IRDescOps::kKey_STANDARD << 24) |
      (static_cast<uint32_t>(intPolicy) << 20) |
      (IRDescOps::kBranch_Never << 16) |
      (static_cast<uint32_t>(syncMatch) << 14) | (reqCount & 0xFFFF);
...
void ASOHCIIRProgramBuilder::BuildInputLast(
    uint32_t bufferPA, uint32_t reqCount, IRIntPolicy intPolicy,
    IRSyncMatch syncMatch, bool includeHeader, bool includeTimestamp,
    uint32_t branchAddress, IRDesc::Descriptor *outDesc) {
  if (!outDesc)
    return;
  ...
  // Build quadlet 0: cmd | key | i | b | w fields (§10.1, Table 10-1)
  outDesc->quad[0] =
      (IRDescOps::kCmd_INPUT_LAST << 28) | (IRDescOps::kKey_STANDARD << 24) |
      (static_cast<uint32_t>(intPolicy) << 20) |
...
void ASOHCIIRProgramBuilder::BuildDualBuffer(
    const IRDualBufferInfo &info, IRIntPolicy intPolicy, IRSyncMatch syncMatch,
    uint32_t branchAddress, uint8_t zValue,
  ...
  outDesc->SetControl(intPolicy == IRIntPolicy::kAlways,
                      ...
                      static_cast<uint8_t>(syncMatch), info.firstSize);
...
void ASOHCIIRProgramBuilder::LinkDescriptors(IRDesc::Descriptor *descriptors,
                                             uint32_t count, uint32_t basePA) {
  // Standard INPUT descriptors don't use explicit linking like OUTPUT
  // descriptors The hardware processes them sequentially based on the branch
  // control field Only need to ensure proper branch control is set (done in
  // BuildInputMore/BuildInputLast)
}

void ASOHCIIRProgramBuilder::LinkDualBufferDescriptors(
    IRDesc::DualBufferDescriptor *descriptors, uint32_t count,
    uint32_t basePA) {
