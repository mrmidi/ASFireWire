...
//
// Spec anchors:
//   Host interrupt + IsoRx event/mask registers: OHCI 1.1 Chapter 6 (event bits
//   demux; not IR-specific semantics) IR DMA programs & descriptor usage: §10.1
...
kern_return_t ASOHCIIRContext::Initialize(IOPCIDevice *pci, uint8_t barIndex,
                                          uint32_t ctxIndex) {
  if (!pci)
  ...
  return ASOHCIContextBase::Initialize(pci, barIndex,
                                       ASContextKind::kIR_Receive, offs);
}
...
kern_return_t ASOHCIIRContext::Start() {
  ...
  os_log(ASLog(), "IR%u: Start deferred (will run on first enqueue)",
         _ctxIndex);
...
void ASOHCIIRContext::ApplyChannelFilter(const IRChannelFilter &filter) {
  ...
  os_log(
      ASLog(),
      "IR%u: Channel filter applied - channel=%u, tag=%u, sync=%u, multiCh=%s",
...
kern_return_t ASOHCIIRContext::EnqueueStandard(const IRDesc::Program &program,
  ...
  os_log(ASLog(),
         "IR%u: Enqueued standard program - mode=%u, headPA=0x%x, Z=%u",
...
kern_return_t
ASOHCIIRContext::EnqueueDualBuffer(const IRProgram::DualBufferProgram &program,
                                   const IRQueueOptions &opts) {
  if (!_pci)
    return kIOReturnNotReady;
  ...
  os_log(ASLog(),
         "IR%u: Enqueued dual-buffer program - headPA=0x%x, Z=%u, firstSize=%u",
...
void ASOHCIIRContext::OnInterruptRx() {
  ...
  os_log(ASLog(), "IR%u: Interrupt processed - outstanding=%u", _ctxIndex,
         _outstanding);
}

void ASOHCIIRContext::SetCompletionCallback(
    void (*callback)(const IRCompletion &, void *), void *context) {
...
kern_return_t ASOHCIIRContext::RefillBuffers() {
  ...
  // This would allocate new receive buffers and link them into the descriptor
  // chain
...
kern_return_t ASOHCIIRContext::ConfigureContextMatch(uint8_t channel,
                                                     uint8_t tag,
                                                     uint8_t sync) {
  if (!_pci)
    return kIOReturnNotReady;
  ...
  uint32_t matchValue =
      ((sync & 0xF) << 8) | ((tag & 0x3) << 6) | (channel & 0x3F);
...
kern_return_t ASOHCIIRContext::SetMultiChannelMode(bool enable,
                                                   uint64_t channelMask) {
  if (!_pci)
    return kIOReturnNotReady;
  if (_ctxIndex != 0)
    return kIOReturnBadArgument; // Multi-channel only on context 0

  if (enable) {
    WriteContextSet(kOHCI_IR_MultiChannelMode);
    // TODO: Program the channel mask registers (implementation-specific)
    os_log(ASLog(), "IR%u: Multi-channel mode enabled, mask=0x%llx", _ctxIndex,
           channelMask);
  } else {
...
ASOHCIIRContext::ReceiveBuffer *ASOHCIIRContext::GetNextFreeBuffer() {
  if (_bufferRingFull)
    return nullptr;
  if (_bufferHead == _bufferTail && _buffers[_bufferHead].inUse)
    return nullptr;
