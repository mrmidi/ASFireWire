...
// Builds INPUT_MORE/INPUT_LAST and DUALBUFFER descriptor chains for IR receive
// modes.
//
...
public:
  ...
  kern_return_t BuildBufferFillProgram(uint32_t bufferPA, uint32_t bufferSize,
  ...
  kern_return_t
  BuildDualBufferProgram(const IRDualBufferInfo &info, uint32_t descriptorCount,
                         const IRQueueOptions &opts,
...
private:
  ...
  void BuildInputMore(uint32_t bufferPA, uint32_t reqCount,
                      IRIntPolicy intPolicy, IRSyncMatch syncMatch,
                      bool includeHeader, IRDesc::Descriptor *outDesc);

  // Build INPUT_LAST descriptor (§10.1, Table 10-1)
  void BuildInputLast(uint32_t bufferPA, uint32_t reqCount,
                      IRIntPolicy intPolicy, IRSyncMatch syncMatch,
                      bool includeHeader, bool includeTimestamp,
                      uint32_t branchAddress, IRDesc::Descriptor *outDesc);

  // Build DUALBUFFER descriptor (§10.2.3, Table 10-2)
  void BuildDualBuffer(const IRDualBufferInfo &info, IRIntPolicy intPolicy,
                       IRSyncMatch syncMatch, uint32_t branchAddress,
                       uint8_t zValue, IRDesc::DualBufferDescriptor *outDesc);

  // Link descriptors into a program chain
  void LinkDescriptors(IRDesc::Descriptor *descriptors, uint32_t count,
                       uint32_t basePA);
