...
void Topology::AddOrUpdateNode(const SelfID::AlphaRecord& rec)
{
  ...
  if (it == _phyIndex.end()) {
    ...
    for (int i = 0; i < 16; ++i) {
      // static_cast is safe due to aligned enums between PortState and SelfID::PortCode
...
{
  // IEEE 1394-2008 Annex P: Construct explicit parent/child edges from port states
  // Each Parent port on node A should match with exactly one Child port on node B
  ...
  // For each node with Parent ports, find corresponding Child ports on other nodes
  for (size_t i = 0; i < _nodes.size(); ++i) {
    ...
    for (int portA = 0; portA < 16; ++portA) {
      if (nodeA.ports[portA] == PortState::Parent) {
        ...
        if (!foundMatch) {
          orphanedPorts++;
          _info.warnings.push_back("Orphaned Parent port on PHY " + std::to_string(nodeA.phy.value) + " port " + std::to_string(portA));
        }
      }
    }
  }
  ...
  if (_nodes.size() > 0 && edgesConstructed != (_nodes.size() - 1)) {
    _info.warnings.push_back("Edge count " + std::to_string(edgesConstructed) + 
                            " != expected " + std::to_string(_nodes.size() - 1) + " for tree structure");
  }
  
  if (orphanedPorts > 0) {
    _info.warnings.push_back("Found " + std::to_string(orphanedPorts) + " orphaned Parent ports");
  }
}
...
void Topology::deriveRoot()
{
  ...
  auto countParent = [](const Node& n){
    uint32_t c = 0; for (auto ps : n.ports) if (ps == PortState::Parent) ++c; return c; };

  for (auto& n : _nodes) n.isRoot = false;

  for (const auto& n : _nodes) {
    if (countParent(n) == 0) {
      _info.rootPhy = n.phy;
      // Mark the first such node as root; multiple zeros indicates inconsistency.
      break;
    }
  }
  if (!_info.rootPhy.valid()) {
    ...
    for (const auto& n : _nodes) {
      if (n.contender) { _info.rootPhy = n.phy; break; }
    }
  }
  if (_info.rootPhy.valid()) {
    auto it = _phyIndex.find(_info.rootPhy.value);
    if (it != _phyIndex.end()) _nodes[it->second].isRoot = true;
  }
}
...
void Topology::Finalize()
{
  ...
  for (const auto& n : _nodes) {
    for (auto ps : n.ports) {
      if (ps == PortState::Parent)  ++parents;
      else if (ps == PortState::Child)   ++children;
    }
  }
  if (parents != children) {
    _info.warnings.push_back("Parent/Child port counts mismatch");
  }
  // In a tree of N nodes, there should be N-1 links; each link contributes one parent and one child
  // across nodes, so parents == children == N-1 ideally.
  if (_nodes.size() > 0) {
...
{
  if (!_info.rootPhy.valid()) return nullptr;
  auto it = _phyIndex.find(_info.rootPhy.value);
  if (it == _phyIndex.end()) return nullptr;
...
const Topology::Node* Topology::FindByNodeId(NodeId id) const
{
  ...
  if (static_cast<size_t>(id.value) >= _nodes.size()) return nullptr;
  return &_nodes[id.value];
}

void Topology::ForEachNode(const std::function<void(const Node&)>& fn) const
{
  for (const auto& n : _nodes) fn(n);
}

bool Topology::HasCycles() const {
  // Without explicit adjacency, use a coarse check: if total links >= N then cycles likely.
  uint32_t parents = 0;
  for (const auto& n : _nodes) for (auto ps : n.ports) if (ps == PortState::Parent) ++parents;
  return (_nodes.size() > 0) && (parents >= _nodes.size());
}

uint8_t Topology::MaxHopsFromRoot() const {
  const Node* root = Root();
  if (!root || _nodes.empty()) return 0;
  ...
  while (queueStart < queue.size()) {
    ...
    for (NodeId childId : current->children) {
      if (childId.valid() && childId.value < _nodes.size() && !visited[childId.value]) {
...
bool Topology::IsConsistent() const {
  ...
  for (const auto& n : _nodes) {
    if (n.isRoot) ++roots;
    for (auto ps : n.ports) {
      if (ps == PortState::Parent) ++parentPorts;
      else if (ps == PortState::Child) ++childPorts;
    }
  }
  
  // Tree structure validation
  if (roots != 1) return false;                            // Must have exactly one root
  if (parentPorts != childPorts) return false;             // Reciprocal parent/child ports
  if (parentPorts != (_nodes.size() - 1)) return false;    // N-1 edges for N nodes
  
  // Verify bidirectional edge reciprocity (IEEE 1394 Annex P requirement)
  for (const auto& node : _nodes) {
    // For each parent relationship, verify the parent lists this node as a child
    for (NodeId parentId : node.parents) {
      const Node* parent = FindByNodeId(parentId);
      if (!parent) return false; // Invalid parent reference
      ...
      if (!foundReciprocal) return false; // Missing reciprocal edge
    }
    ...
    for (NodeId childId : node.children) {
      ...
      if (!child) return false; // Invalid child reference
...
{
  auto it = _phyIndex.find(phy.value);
  if (it == _phyIndex.end()) return false;
  _nodes[it->second].rom = props;
  return true;
}

static inline const char* speedStr(SelfID::LinkSpeed s) {
  switch (s) { case SelfID::LinkSpeed::S100: return "S100"; case SelfID::LinkSpeed::S200: return "S200"; case SelfID::LinkSpeed::S400: return "S400"; default: return "RES"; }
}
...
void Topology::Log() const
{
  ...
  os_log(ASLog(), "ASOHCI: === Topology Snapshot === gen=%u nodes=%lu rootPhy=%u hops=%u consistent=%d warnings=%lu",
         bi.generation, (unsigned long)nodes, root ? root->phy.value : 0xFF, hops, ok ? 1 : 0, (unsigned long)bi.warnings.size());
  if (!bi.warnings.empty()) {
    for (const auto& w : bi.warnings) os_log(ASLog(), "ASOHCI:  warn: %{public}s", w.c_str());
  }
  for (const auto& n : _nodes) {
    uint32_t pParents = 0, pChildren = 0, present = 0, active = 0;
    char portLine[17]; portLine[16] = '\0';
    for (int i = 0; i < 16; ++i) {
      char c = '-';
      switch (n.ports[i]) {
        case PortState::NotPresent: c = '-'; break;
        case PortState::NotActive:  c = '.'; ++present; break;
        case PortState::Parent:     c = 'P'; ++present; ++active; ++pParents; break;
        case PortState::Child:      c = 'C'; ++present; ++active; ++pChildren; break;
      }
      portLine[i] = c;
    }
    os_log(ASLog(), "ASOHCI:  node phy=%u id=%u %s L=%u gap=%u sp=%s cont=%u pwr=%u init=%u ports[%u/%u]: %s",
           n.phy.value, n.nodeId.valid() ? n.nodeId.value : 0xFF, (n.isRoot?"(root)":""), n.linkActive?1:0, n.gapCount,
           speedStr(n.speed), n.contender?1:0, n.powerClass, n.initiated?1:0, active, present, portLine);
    // Print adjacency by PHY where possible
    if (!n.parents.empty() || !n.children.empty()) {
      // Build small lists of PHY ids
      char buf[128]; buf[0] = '\0'; size_t off = 0;
      if (!n.parents.empty()) {
        off += snprintf(buf+off, sizeof(buf)-off, " parents:");
        for (auto pid : n.parents) {
          const Node* pn = FindByNodeId(pid);
          off += snprintf(buf+off, sizeof(buf)-off, " %u", pn ? pn->phy.value : 0xFF);
        }
      }
      if (!n.children.empty()) {
        ...
        for (auto cid : n.children) {
          ...
          off += snprintf(buf+off, sizeof(buf)-off, " %u", cn ? cn->phy.value : 0xFF);
        }
      }
