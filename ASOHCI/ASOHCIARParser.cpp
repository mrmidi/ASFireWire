...
static inline ARTCode DecodeARTCode(uint32_t hdr1_host) {
    ...
    switch (nib) {
        case 0x0: return ARTCode::kWriteQuadlet;
        case 0x1: return ARTCode::kWriteBlock;
        case 0x2: return ARTCode::kWriteResponse;
        case 0x4: return ARTCode::kReadQuadlet;
        case 0x5: return ARTCode::kReadBlock;
        case 0x6: return ARTCode::kReadResponse;
        case 0x0C: return ARTCode::kCycleStart;   // some silicon reports cycle start
        case 0x07: return ARTCode::kLock;         // lock family
        default:   return ARTCode::kUnknown;
    }
}
...
uint32_t ASOHCIARParser::HeaderSize(const uint8_t* /*bytes*/, uint32_t /*len*/) const
{
...
bool ASOHCIARParser::Parse(const ARPacketView& view, ARParsedPacket* out) const
{
    if (!out) return false;
    *out = ARParsedPacket{};
    if (!view.data || view.length < 8) return false;
    ...
    if (view.length < hdrBytes) hdrBytes = 8;
