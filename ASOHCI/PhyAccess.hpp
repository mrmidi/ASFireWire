...
// Encapsulates serialized access to OHCI PhyControl register using
// IORecursiveLock.
...
private:
  ...
  bool
  waitForWriteComplete(uint32_t timeoutIterations); // busy poll small bounded
...
public:
  ...
  kern_return_t updatePhyRegisterWithMask(uint8_t reg, uint8_t value,
                                          uint8_t mask);
};
