...
//
// Spec refs: OHCI 1.1 §3.1.2 (CommandPtr/Z nibble), AT §7.1 (program/list rules),
//            AR §8.1 (program/list rules). Descriptors are 16-byte units, 32-bit DMA.
//
...
public:
    ...
    struct Block {
        ...
        uint8_t  zValue          = 0;  // Z nibble for CommandPtr if this block is used as a single program
        bool     valid           = false;
    };

    // Allocate ‘unitCount’ 16-byte units (2..8 typical for a single packet program).
