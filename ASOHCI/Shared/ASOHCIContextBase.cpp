...
//
// Spec refs: OHCI 1.1 §3.1 (Context registers), §3.1.1 (run/active/dead/wake),
// §3.1.2 (CommandPtr)
//
...
kern_return_t ASOHCIContextBase::Initialize(IOPCIDevice *pci, uint8_t barIndex,
                                            ASContextKind kind,
  ...
  os_log(ASLog(),
         "ASOHCIContextBase: Init kind=%u base=0x%x set=0x%x clr=0x%x cmd=0x%x",
         static_cast<unsigned>(_kind), _offs.contextBase,
         _offs.contextControlSet, _offs.contextControlClear, _offs.commandPtr);
...
kern_return_t ASOHCIContextBase::Start() {
  ...
  os_log(ASLog(), "ASOHCIContextBase: Started (kind=%u)",
...
kern_return_t ASOHCIContextBase::WriteCommandPtr(uint32_t descriptorAddress,
                                                 uint8_t zNibble) {
  if (!_pci)
    return kIOReturnNotReady;
