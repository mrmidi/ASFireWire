...
//
// Spec refs: OHCI 1.1 §3.1 (Context Registers overview), §3.1.2 (CommandPtr
// format)
//
...
// Minimal completion view that both AT/AR can surface from
// OUTPUT_LAST/INPUT_LAST (xferStatus/timeStamp encoding differs by descriptor
// type; callers decode as needed) Spec refs: §3.1.2 (CommandPtr & Z); AT §7.1.5
// (completion fields); AR §8.1.5 (completion fields).
struct ASCompletionMini {
  uint16_t xferStatus =
      0; // lower 16 of “status/timestamp” quadlet used by the context
  uint16_t timeStamp =
      0; // upper or lower 16 depending on desc type (caller knows side)
};
