...
bool ASOHCIConfigROM::buildFromHardware(uint32_t busOptions, uint32_t guidHi, uint32_t guidLo,
                                        bool includeRootDirectory, bool includeNodeCapsEntry)
{
    _quads.clear();
    _rootDirStart = 0;
...
uint32_t ASOHCIConfigROM::makeDirEntry(EntryType type, uint8_t key, uint32_t value)
{
    return ((uint32_t)type << 30) | ((uint32_t)(key & 0x3F) << 24) | (value & 0x00FFFFFFu);
}
...
uint16_t ASOHCIConfigROM::computeCRC16(const std::vector<uint32_t>& quads, size_t startIdx, size_t quadCount)
{
    uint16_t crc = 0;
...
void ASOHCIConfigROM::buildBIB(uint32_t busOptions, uint32_t guidHi, uint32_t guidLo)
{
    // General BIB: header + 4 quadlets (bus_name, node_capabilities, guid_hi, guid_lo)
    _quads.reserve(8);
    ...
    // Header fields
    const uint8_t bus_info_length = 4; // number of quadlets following header in BIB
    const uint8_t crc_length = 4;      // quadlets covered by BIB CRC (quads 1..4)
    uint16_t bib_crc = computeCRC16(_quads, 1, crc_length);
    _quads[0] = ((uint32_t)bus_info_length << 24) | ((uint32_t)crc_length << 16) | (uint32_t)bib_crc;
}
