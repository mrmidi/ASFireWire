...
// Based on OHCI 1.1 Specification §7.5 (Responses follow IEEE1394a rules, not
// fairness)
//
...
kern_return_t ASOHCIATResponseContext::Initialize(IOPCIDevice *pci,
                                                  uint8_t barIndex) {
  ...
  return ASOHCIContextBase::Initialize(pci, barIndex,
                                       ASContextKind::kAT_Response, offs);
}

void ASOHCIATResponseContext::ApplyPolicy(const ATRetryPolicy &retry,
...
kern_return_t ASOHCIATResponseContext::Enqueue(const ATDesc::Program &program,
  ...
  if (!wasActive) {
    OHCI_MEMORY_BARRIER(); // Ensure all descriptor writes visible before
                           // programming CommandPtr (OHCI §7.1)
    ...
    if ((cc & kOHCI_ContextControl_active) == 0) {
      OHCI_MEMORY_BARRIER(); // Ensure all descriptor writes visible before
                             // programming CommandPtr (OHCI §7.1)
      WriteCommandPtr(program.headPA, program.zHead);
      OHCI_MEMORY_BARRIER(); // Ensure CommandPtr write is globally visible
                             // before run (OHCI §7.1)
      Wake();
    } else {
...
uint32_t ASOHCIATResponseContext::DrainCompletions(uint32_t maxToDrain) {
  ...
  if (hasCompletion && completed < maxToDrain)
    completed = 1;
  return completed;
}
