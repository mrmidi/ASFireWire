...
kern_return_t SelfIDManager::Initialize(::IOPCIDevice *pci, uint8_t barIndex,
                                        uint32_t bufferBytes) {
  if (!pci || bufferBytes == 0)
  ...
  os_log(ASLog(), "ASOHCI: SelfID map addr=0x%llx len=0x%llx",
         (unsigned long long)_map->GetAddress(),
         (unsigned long long)_map->GetLength());
  ...
  kr = _dma->PrepareForDMA(kIODMACommandPrepareForDMANoOptions, _buf, 0,
                           bufferBytes, &flags, &segCount, segs);
  os_log(ASLog(),
         "ASOHCI: SelfID DMA prepare status=0x%08x segs=%u addr0=0x%llx "
         "len0=0x%llx",
         kr, segCount,
  ...
  // Store only the first segment pointer (address/length) by allocating one
  // slot.
  ...
  *_seg = segs[0];
  ...
  uint32_t rb = 0;
  _pci->MemoryRead32(_bar, kOHCI_SelfIDBuffer, &rb);
...
void SelfIDManager::Teardown() {
  ...
  if (_seg) {
    IOFree(_seg, sizeof(::IOAddressSegment));
    _seg = nullptr;
  }
...
kern_return_t SelfIDManager::Arm(bool clearCount) {
  ...
  _pci->MemoryWrite32(_bar, kOHCI_LinkControlSet,
...
void SelfIDManager::programSelfIDBuffer() {
  if (_pci && _seg) {
    _pci->MemoryWrite32(_bar, kOHCI_SelfIDBuffer,
                        static_cast<uint32_t>(_seg->address));
  }
}
...
void SelfIDManager::verifyGenerationAndDispatch(uint32_t countReg) {
  ...
  uint32_t sizeQuads =
      (countReg & kOHCI_SelfIDCount_selfIDSize) >> 2; // size in bytes >>2
  bool error = (countReg & kOHCI_SelfIDCount_selfIDError) != 0;
  os_log(ASLog(), "ASOHCI: SelfIDComplete: gen=%u sizeQ=%u error=%u", gen1,
         sizeQuads, error ? 1 : 0);
  ...
  uint32_t lenQuads =
      static_cast<uint32_t>(_map->GetLength() / sizeof(uint32_t));
  if (sizeQuads > lenQuads)
    sizeQuads = lenQuads; // clamp

  os_log(ASLog(),
         "ASOHCI: SelfIDComplete: Decoding buffer len=%u quads, processing %u "
         "quads",
         lenQuads, sizeQuads);
  ...
  // Decode now (can be heavy). If needed, the caller can route to default queue
  // instead.
  ...
  os_log(ASLog(),
         "ASOHCI: SelfID decode result: nodes=%lu integrityOk=%d warnings=%lu",
  ...
  uint32_t count2 = 0;
  _pci->MemoryRead32(_bar, kOHCI_SelfIDCount, &count2);
  ...
  if (gen1 == gen2) {
    ...
    os_log(ASLog(),
           "ASOHCI: Generation changed during decode (gen1=%u gen2=%u)", gen1,
           gen2);
  }
