...
kern_return_t ConfigROMManager::Initialize(::IOPCIDevice *pci, uint8_t barIndex,
                                           uint32_t busOptions, uint32_t guidHi,
                                           uint32_t guidLo, uint32_t romBytes) {
  if (!pci || romBytes == 0)
  ...
  *_seg = segs[0];
  ...
  os_log(ASLog(), "ASOHCI: ConfigROM built: vendor=0x%06x EUI64=%08x%08x",
         _vendorId & 0xFFFFFF, (unsigned)((_eui64 >> 32) & 0xFFFFFFFFu),
         (unsigned)(_eui64 & 0xFFFFFFFFu));
  if (_seg) {
    os_log(ASLog(),
           "ASOHCI: ConfigROM mapped IOVA=0x%08x BusOptions=0x%08x "
           "Header(staged)=0x%08x",
...
void ConfigROMManager::buildAndStage(uint32_t busOptions, uint32_t guidHi,
                                     uint32_t guidLo, uint32_t romBytes) {
  if (!_map)
    return;
  // Build ROM (big-endian write) using existing builder
  ASOHCIConfigROM rom;
  rom.buildFromHardware(busOptions, guidHi, guidLo,
                        /*includeRootDirectory*/ true,
                        /*includeNodeCaps*/ true);
...
void ConfigROMManager::programROMMap() {
  if (!_pci || !_seg)
    return;
  _pci->MemoryWrite32(_bar, kOHCI_ConfigROMmap,
...
void ConfigROMManager::CommitOnBusReset() {
  if (!_pci || !_headerNeedsCommit || _headerQuad == 0)
    return;
  _pci->MemoryWrite32(_bar, kOHCI_BusOptions, _busOptions);
...
void ConfigROMManager::Dump(const char * /*label*/) const {
  // Intentionally left as a lightweight no-op to avoid introducing a shared hex
  // dump dependency.
}

uint64_t ConfigROMManager::ROMIOVA() const {
