...
enum class ITSpeed : uint8_t {
  ...
  S200 = 1,
  S400 = 2,
  S800 = 3, // extend if your silicon supports more
};

// IT interrupt policy for OUTPUT_LAST* descriptors 'i' field (OHCI §9.1.3,
// §9.1.4, Table 9-2, Table 9-3)
...
struct ITQueueOptions {
  // Per-packet interrupt policy (IT-specific 'i' bits in OUTPUT_LAST*, OHCI
  // Table 9-3)
  ...
  uint8_t startOnCycle =
      0; // 7-bit cycle number; honored only if cycleMatchEnable
...
struct ITPolicy {
  ...
  bool dropIfLate = true; // Software policy only: if packet missed its cycle,
                          // do not enqueue retroactively
  uint32_t underrunBudgetUs =
      0; // controller-specific: how soon to re-arm after underrun
  ITIntPolicy defaultInterruptPolicy =
      ITIntPolicy::kOnCompletion; // Default interrupt policy for packets
};
