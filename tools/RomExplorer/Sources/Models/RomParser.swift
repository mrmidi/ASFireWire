...
public struct RomParser {
    public static func parse(fileURL: URL) throws -> RomTree {
        ...
        let cache = try RomCache(fileData: fileData)
    ...
    // makeBigEndian is moved into RomCache; sliceBusInfo replaced by cache.busInfoRaw
    ...
    private static func readLeafSafe(cache: RomCache, leafStartQ: Int, keyId: UInt8) -> RomValue {
        ...
        if leafStartQ >= cache.quadletCount {
            DebugLog.warn("Leaf OOB leafStartQ=\(leafStartQ) totalQ=\(cache.quadletCount)")
    ...
    let quadlets = Int((meta & 0xffff_0000) >> 16)
        ...
        if payloadQ > cache.quadletCount {
            DebugLog.warn("Leaf payload OOB payloadQ=\(payloadQ) quadlets=\(quadlets) totalQ=\(cache.quadletCount)")
            return .leafPlaceholder(offset: leafStartQ * 4)
        }
    ...
    switch KeyType(rawValue: keyId) {
        case .descriptor:
            ...
            if specId == 0, descType == 0, remain.count >= 4 {
                ...
                // charset and language may vary by device; don't over-constrain
            ...
            // Not recognized as textual descriptor; return raw payload
    ...
    private static func bestEffortText(from data: Data) -> String? {
        // ASCII sequences >= 4
        let ascii = extractASCII(from: data)
        // UTF-16BE sequences >= 4 chars
        let u16 = extractUTF16BE(from: data)
        let candidates = (ascii + u16).sorted { $0.count > $1.count }
        return candidates.first
    }

    private static func extractASCII(from data: Data) -> [String] {
        var out: [String] = []
        var buf: [UInt8] = []
        for b in data {
            if b >= 0x20 && b <= 0x7e {
                ...
                if buf.count >= 4, let s = String(bytes: buf, encoding: .ascii) { out.append(s) }
    ...
    private static func extractUTF16BE(from data: Data) -> [String] {
        ...
        while i + 1 < data.count {
            ...
            if hi == 0x00 && lo >= 0x20 && lo <= 0x7e {
                scalars.append(UInt16(lo))
                i += 2
            } else {
                if scalars.count >= 4 {
                    let u = Data(scalars.flatMap { [UInt8(0x00), UInt8($0 & 0xff)] })
                    if let s = String(data: u, encoding: .utf16BigEndian) { out.append(s) }
                }
    ...
    private static func readDirectory(cache: RomCache, startQ: Int) throws -> [DirectoryEntry] {
        ...
        let quadlets = Int((meta & 0xffff_0000) >> 16)
        let entryCount = quadlets // includes all quadlets after header; last is CRC
        let maxReadable = max(0, cache.quadletCount - (startQ + 1))
        let count = min(entryCount, maxReadable)
        DebugLog.info("Dir startQ=\(startQ) lenQ=\(entryCount) clamp=\(count) quadlets=\(cache.quadletCount)")
        ...
        while i < count {
            ...
            case .directory:
                ...
                if childStartQ + 1 <= cache.quadletCount {
                    ...
                    DebugLog.warn("Subdir OOB childStartQ=\(childStartQ) totalQ=\(cache.quadletCount)")
