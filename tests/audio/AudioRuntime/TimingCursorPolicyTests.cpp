...
TEST(TimingCursorPolicyTests, DICE48kBlockingValues) {
    ...
    EXPECT_EQ(policy.PacketLeadFrames(AudioDirection::Output), 384U);
    EXPECT_EQ(policy.StartupLeadFrames(AudioDirection::Output), 384U);
    ...
    EXPECT_EQ(policy.CursorResyncDeadbandFrames(), 64U);
