...
TEST(DuplexRestartSessionTests, ClearRestartProgressPreservesIntentButClearsExecutionState) {
    ...
    // Stage 2b Phase C: ClearRestartProgress is the rollback-ledger reset only.
    // Lifecycle transitions belong to the journal's terminal helpers
    // (EnterIdle/EnterFailed) — single authority over the lifecycle state.
    EXPECT_EQ(KindOf(session.lifecycle), DuplexLifecycleKind::Running); // preserved
