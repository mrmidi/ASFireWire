...
TEST(SBP2LoginSessionTests, QueuedUnsolicitedStatusEnableUsesDerivedAddressAfterLogin) {
    SessionRig rig;
...
TEST(SBP2LoginSessionTests, LoginWritesBusyTimeoutRegister) {
    SessionRig rig;
...
TEST(SBP2LoginSessionTests, ReconnectReplaysBusyTimeoutRegister) {
    SessionRig rig;
    rig.LoginSuccessfully();
...
TEST(SBP2LoginSessionTests, ImmediateORBWriteRetryExhaustionFailsActiveCommandAndResetsFetchAgent) {
    SessionRig rig;
    rig.LoginSuccessfully();
