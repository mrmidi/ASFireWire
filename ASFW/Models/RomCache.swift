...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "ASFW", category: "RomCache")
...
struct RomCache {
    ...
    func quadlet(at q: Int) throws -> UInt32 {
        ...
        guard q >= 0 && q < quadletCount else {
            logger.error("quadlet OOB @\(q) (quadlets=\(self.quadletCount), off=\(q &* 4), bytes=\(self.data.count))")
    ...
    func readBytes(quadletStart q: Int, quadletLength lq: Int) throws -> Data {
        ...
        guard q >= 0, lq >= 0 else {
            logger.error("slice negative params q=\(q), lq=\(lq)")
        ...
        guard q <= quadletCount else {
            logger.error("slice OOB start @\(q) (quadlets=\(self.quadletCount))")
        ...
        if end < off &+ len {
            logger.warning("slice clamped q=\(q) lq=\(lq) off=\(off) len=\(len) end=\(end) bytes=\(self.data.count)")
        }
