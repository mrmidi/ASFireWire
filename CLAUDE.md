...
- **CoreAudio HAL semantics** (what a buffer size / timestamp / latency property *means*, as opposed to its type) → the local `coreaudio-api` mailing-list archive at `/Users/mrmidi/DEV/MailingList/wayback-machine-downloader` (SQLite + FTS5, ~36k messages, `is_apple_author` flag; `./mail_tool.py search "<terms>" --apple-only`). Statements by the HAL authors there are behavioural ground truth that the headers and `ctx7` do **not** carry. Findings already extracted — the client-buffer vs driver-ring/ZTS domain split, what the IO callback's `hostTime` is, and what `kAudioDevicePropertyLatency` may contain — are written up in `documentation/COREAUDIO_HAL_TIMING_DOMAINS.md`. **Read that before reasoning about any frame count that crosses the HAL seam**: several in-tree bugs came from comparing two quantities that were both "frames" but lived in different domains.
